# this_file: src/d361/core/models.py
"""
...
class Article(BaseModel):
    ...
    # Core identifiers
    id: int | str = Field(
        ..., description="Unique article identifier (integer or UUID string)"
    )
    ...
    content_type: ContentType = Field(
        ContentType.ARTICLE, description="Type of content"
    )
    ...
    category_id: int | str | None = Field(
        None, description="Parent category identifier (integer or UUID string)"
    )
    ...
    author: str | None = Field(None, description="Author name (convenience field)")
    author_id: int | str | None = Field(
        None, description="Author identifier (integer or UUID string)"
    )
    ...
    meta_description: str = Field(
        "", max_length=300, description="SEO meta description"
    )
    tags: list[str] = Field(default_factory=list, description="Article tags")

    # Document360-specific fields
    version_id: int | str | None = Field(
        None, description="Version identifier (integer or UUID string)"
    )
    ...
    is_hidden: bool = Field(
        False, description="Whether article is hidden from navigation"
    )
    ...
    def validate_id(cls, v: Any) -> Any:
        """Reject None, empty strings, and whitespace-only strings as IDs."""
        if v is None:
            raise ValueError("id must not be None")
        if isinstance(v, str) and not v.strip():
            raise ValueError("id must not be empty or whitespace")
        if isinstance(v, list):
            raise ValueError("id must be a string or integer, not a list")
        return v

    @field_validator("tags", mode="before")
    @classmethod
    def coerce_tags_none(cls, v: Any) -> Any:
    ...
    def auto_generate_slug(self) -> Article:
        """Auto-generate slug from title if not provided."""
        if not self.slug and self.title:
            object.__setattr__(self, "slug", _slugify(self.title))
        return self
...
class Category(BaseModel):
    ...
    # Core identifiers
    id: int | str = Field(
        ..., description="Unique category identifier (integer or UUID string)"
    )
    ...
    article_count: int = Field(
        0, ge=0, description="Number of articles in this category"
    )
    subcategory_count: int = Field(
        0, ge=0, description="Number of direct subcategories"
    )

    @model_validator(mode="after")
    def auto_generate_slug(self) -> Category:
...
class ProjectVersion(BaseModel):
    ...
    id: int | str = Field(..., description="Unique version identifier")
    ...
    # Project information
    project_id: int | str | None = Field(None, description="Parent project identifier")
    ...
    webhook_urls: list[str] = Field(
        default_factory=list, description="Webhook endpoints"
    )
...
class SearchResult(BaseModel):
    ...
    results: list[Article | Category] = Field(
    ...
    facets: dict[str, dict[str, int]] = Field(
        default_factory=dict, description="Search facets"
    )
