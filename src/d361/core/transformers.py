...
class ModelTransformer:
    ...
    def from_api_response(
        data: dict[str, Any], content_type: str = "article"
    ) -> Article | Category | ProjectVersion:
    ...
    def _transform_api_article(data: dict[str, Any]) -> Article:
        ...
        updated_at = (
            ModelTransformer._parse_timestamp(
                data.get("updated_at")
        ...
        raw_status = (
            data.get("status") or data.get("Status") or data.get("is_published")
        )
        ...
        return Article(
            ...
            content=str(
                data.get("content") or data.get("Content") or data.get("body") or ""
            ),
            content_markdown=str(
                data.get("content_markdown") or data.get("markdown") or ""
            ),
            excerpt=str(
                data.get("excerpt") or data.get("Excerpt") or data.get("summary") or ""
            ),
            category_id=int(
            ...
            order=int(
                data.get("order") or data.get("Order") or data.get("sort_order") or 0
            ),
            status=status,
            ...
            meta_title=str(
                data.get("meta_title")
                or data.get("MetaTitle")
                or data.get("seo_title")
                or ""
            ),
            meta_description=str(
                data.get("meta_description") or data.get("MetaDescription") or ""
            ),
            tags=ModelTransformer._parse_tags(
                data.get("tags") or data.get("Tags") or []
            ),
            version_id=data.get("version_id") or data.get("VersionId"),
            language_code=str(
                data.get("language_code") or data.get("LanguageCode") or "en"
            ),
    ...
    def _transform_api_category(data: dict[str, Any]) -> Category:
        ...
        return Category(
            ...
            metadata=data.get("metadata", {}),
            article_count=int(
                data.get("article_count") or data.get("ArticleCount") or 0
            ),
            subcategory_count=int(
                data.get("subcategory_count") or data.get("SubcategoryCount") or 0
            ),
        )
    ...
    def _transform_api_project(data: dict[str, Any]) -> ProjectVersion:
        ...
        return ProjectVersion(
            ...
            name=str(data.get("name") or data.get("Name") or ""),
            version_number=str(
                data.get("version_number") or data.get("VersionNumber") or "1.0.0"
            ),
            is_default=bool(data.get("is_default") or data.get("IsDefault", False)),
            project_id=int(
                data.get("project_id") or data.get("ProjectId") or project_id
            ),
            ...
            site_description=str(
                data.get("site_description") or data.get("SiteDescription") or ""
            ),
            metadata=data.get("metadata", {}),
        )
    ...
    def _transform_archive_article(data: dict[str, Any]) -> Article:
        ...
        if not article_id:
            ...
            if match:
                ...
                raise ValueError(
                    "Article ID is required but not found in archive metadata"
                )
        ...
        return Article(
            ...
            content=str(
                data.get("content") or data.get("body") or data.get("html") or ""
            ),
            content_markdown=str(
                data.get("markdown") or data.get("content_markdown") or ""
            ),
    ...
    def _transform_archive_category(data: dict[str, Any]) -> Category:
        ...
        category_id = (
            data.get("id") or data.get("categoryId") or data.get("category_id")
        )
        if not category_id:
            raise ValueError(
                "Category ID is required but not found in archive metadata"
            )
        ...
        return Category(
            ...
            created_at=ModelTransformer._parse_timestamp(
                data.get("created") or data.get("createdAt")
            ),
            updated_at=ModelTransformer._parse_timestamp(
                data.get("modified") or data.get("updatedAt")
            ),
    ...
    def _transform_archive_project(data: dict[str, Any]) -> ProjectVersion:
        ...
        project_id = (
            data.get("id") or data.get("projectId") or data.get("project_id") or 1
        )
        ...
        return ProjectVersion(
            ...
            version_number=str(
                data.get("version") or data.get("version_number") or "1.0.0"
            ),
    ...
    def to_legacy_format(
        model: Article | Category | ProjectVersion, format_name: str
    ) -> dict[str, Any]:
