...
class ValidationHelper:
    ...
    def validate_url(cls, value: Any, allow_relative: bool = False) -> str:
        ...
        try:
            ...
            raise ValidationException(
                f"Invalid URL format: {value}", value=value
            ) from e
    ...
    def validate_email(cls, value: Any) -> str:
        ...
        try:
            ...
            raise ValidationException(
                f"Invalid email format: {value}", value=value
            ) from e
    ...
    def validate_version(cls, value: Any) -> str:
        ...
        if not cls.VERSION_PATTERN.match(value):
            raise ValidationException(
                "Version must follow semantic versioning (e.g., 1.2.3)", value=value
            )
    ...
    def validate_file_path(
        cls, value: Any, must_exist: bool = False, must_be_file: bool = True
    ) -> Path:
        """
        Validate file path.
        ...
        if isinstance(value, str):
            ...
            raise ValidationException(
                "File path must be a string or Path object", value=value
            )

        if must_exist:
            if not path.exists():
                raise ValidationException(
                    f"Path does not exist: {path}", value=str(path)
                )

            if must_be_file and not path.is_file():
                raise ValidationException(
                    f"Path is not a file: {path}", value=str(path)
                )
    ...
    def validate_date_string(cls, value: Any, format: str = "%Y-%m-%d") -> datetime:
        ...
        try:
            ...
            raise ValidationException(
                f"Invalid date format. Expected {format}, got: {value}", value=value
            ) from e

    @classmethod
    def validate_json_dict(
        cls, value: Any, required_keys: list[str] | None = None
    ) -> dict[str, Any]:
        """
        ...
        if required_keys:
            ...
            if missing_keys:
                raise ValidationException(
                    f"Missing required keys: {missing_keys}", value=list(value.keys())
                )
    ...
    def create_migration_report(model_class: type[BaseModel]) -> dict[str, Any]:
        ...
        if hasattr(model_class, "__validators__"):
            report["recommendations"].append(
                "Update __validators__ to use @field_validator or @model_validator"
            )
...
class ValidatedBaseModel(BaseModel):
    ...
    def from_dict(cls, data: dict[str, Any]) -> ValidatedBaseModel:
        ...
        try:
            ...
            raise ValidationException(
                f"Validation failed for {cls.__name__}", errors=errors
            ) from e
...
class Article(ValidatedBaseModel):
    ...
    url: HttpUrl | None = Field(None, description="Article URL")
    author_email: EmailStr | None = Field(None, description="Author email address")
