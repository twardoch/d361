...
class LogHandlerConfig(BaseModel):
    ...
    sink: str | object = Field(default=sys.stderr, description="Log destination")
    ...
    rotation: str | None = Field(default=None, description="Log rotation policy")
    retention: str | None = Field(default=None, description="Log retention policy")
    compression: str | None = Field(default=None, description="Log compression format")
...
class LoggingConfig(BaseModel):
    ...
    format: LogFormat = Field(
        default=LogFormat.CONSOLE, description="Default log format"
    )
    handlers: list[LogHandlerConfig] = Field(
        default_factory=list, description="Log handlers"
    )
    enable_correlation: bool = Field(
        default=True, description="Enable correlation tracking"
    )
    enable_caller_info: bool = Field(
        default=True, description="Include caller info in logs"
    )
    enable_process_info: bool = Field(
        default=False, description="Include process info in logs"
    )
    ...
    def _add_default_handlers(self) -> None:
        ...
        if self.config.log_file:
            file_handler = LogHandlerConfig(
                ...
                compression="gz",
                serialize=(
                    self.config.format in [LogFormat.JSON, LogFormat.STRUCTURED]
                ),
            )
    ...
    def add_handler(
        ...
        **kwargs,
        ...
        handler_config = LogHandlerConfig(
            ...
            else self._format_for_type(format_type),
            **kwargs,
        )
        self._add_handler(handler_config)
...
def setup_development_logging(log_file: Path | None = None) -> LoggingManager:
