...
class AsyncCache:
    ...
    def __init__(self, config: CacheConfig):
        ...
        self._disk_cache: diskcache.Cache | None = None
...
class BatchProcessor:
    ...
    async def _process_batch(self, processor: Callable[[list[Any]], Any]) -> None:
        ...
        self.metrics.avg_batch_size = (
            self.metrics.batch_total_items / self.metrics.batch_count
        )
...
class PerformanceOptimizer:
    ...
    def get_cache(self, name: str = "default") -> AsyncCache:
        ...
        if name not in self._caches:
            config = CacheConfig(
                directory=self.cache_config.directory / name
                if self.cache_config.directory
                else None,
