...
class ServiceDescriptor:
    ...
    singleton_instance: Any | None = None
...
class ServiceScope:
    ...
    def get_instance(self, service_type: type) -> Any | None:
...
class ServiceContainer:
        ...
        with self._lock:
            ...
            if not inspect.isclass(implementation) and not callable(implementation):
                if lifecycle != ServiceLifecycle.SINGLETON:
                    logger.warning(
                        f"Instance provided for {service_type}, forcing singleton lifecycle"
                    )
            ...
            logger.debug(
                f"Registered service {service_type} with {lifecycle} lifecycle"
            )
    ...
    def _resolve(self, service_type: type[T], resolving_stack: set) -> T:
        ...
        with self._lock:
            ...
            try:
                ...
                logger.debug(
                    f"Created instance of {service_type} with {descriptor.lifecycle} lifecycle"
                )
                return instance
    ...
    def _create_instance(
        self, descriptor: ServiceDescriptor, resolving_stack: set
    ) -> Any:
        """Create a new service instance."""
        # Use factory if provided
        if descriptor.factory:
            factory_args = self._resolve_dependencies(
                descriptor.factory, resolving_stack
            )
        ...
        if inspect.isclass(implementation):
            constructor_args = self._resolve_dependencies(
                implementation.__init__, resolving_stack
            )
    ...
    def _resolve_dependencies(
        self, func: Callable, resolving_stack: set
    ) -> dict[str, Any]:
        ...
        try:
                        ...
                        if len(args) == 2 and type(None) in args:
                            ...
                            try:
                                dependencies[name] = self._resolve(
                                    dependency_type, resolving_stack
                                )
            ...
            raise ServiceRegistrationError(
                f"Cannot resolve dependencies for {func}: {e}"
            )
...
def injectable(
    ...
    def decorator(cls: type[T]) -> type[T]:
        ...
        logger.debug(
            f"Auto-registered service {target_service_type} -> {cls} with {lifecycle} lifecycle"
        )
        return cls
...
def resolve(service_type: type[T], container: ServiceContainer | None = None) -> T:
