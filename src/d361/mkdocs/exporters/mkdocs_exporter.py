...
class MkDocsExporter:
    ...
    def __init__(
        ...
        self.config_generator = ConfigGenerator(
            theme=theme, enable_plugins=enable_plugins
        )
        ...
        logger.info(
            f"Initialized MkDocsExporter for {source} source with {theme} theme"
        )
    ...
    def _initialize_provider(self) -> None:
        ...
        if self.source == "archive":
            ...
            self.provider = ApiProvider(
                api_token=self.api_token, base_url=self.api_base_url
            )

        elif self.source == "hybrid":
            if not self.archive_path or not self.api_token:
                raise ValueError(
                    "Both archive path and API token required for hybrid source"
                )
            ...
            logger.info(
                "Using processed mode - creating minimal provider for stage 2 processing"
            )
    ...
    async def export(self) -> dict[str, Any]:
        ...
        with self.performance_monitor.measure("total_export"):
            try:
                ...
                await self._generate_mkdocs_structure(
                    processed_articles, processed_categories
                )
                ...
                export_results = self._generate_export_results(
                    config_path, validation_results
                )
    ...
    async def _load_content(self) -> tuple[list[Article], list[Category]]:
        ...
        with self.performance_monitor.measure("content_loading"):
            ...
            logger.info(
                f"Loaded {len(articles)} articles and {len(categories)} categories"
            )
    ...
    async def _process_content(self, articles: list[Article]) -> list[Article]:
        ...
        with self.performance_monitor.measure("content_processing"):
            ...
            if self.parallel_processing:
                ...
                tasks = [self._process_single_article(article) for article in articles]
    ...
    async def _generate_mkdocs_structure(
        ...
        navigation = await self.navigation_builder.build_navigation(
            articles, categories
        )
    ...
    async def _write_article_file(self, article: Article, docs_dir: Path) -> None:
        ...
        if hasattr(article, "status") and str(article.status).lower() not in [
            "published",
            "draft",
        ]:
    ...
    async def _generate_homepage(
        self, articles: list[Article], categories: list[Category], docs_dir: Path
    ) -> None:
        ...
        intro_articles = [
            a
            for a in articles
            if "introduction" in a.slug.lower() or "getting-started" in a.slug.lower()
        ]
        [
            a
            for a in articles
            if "interface" in a.slug.lower() or "overview" in a.slug.lower()
        ]
    ...
    async def _generate_category_indexes(
        self, categories: list[Category], docs_dir: Path
    ) -> None:
    ...
    def _generate_export_results(
        self, config_path: Path, validation_results: dict[str, Any]
    ) -> dict[str, Any]:
