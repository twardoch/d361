...
class NavigationBuilder:
    ...
    def __init__(
        ...
        self.breadcrumb_cache: dict[
            str, list[dict[str, str]]
        ] = {}  # article_id -> breadcrumbs
        self.navigation_tree: NavigationItem | None = None

        logger.info(
            f"Initialized NavigationBuilder with Phase 2 enhancements: literate_nav={enable_literate_nav}, smart_ordering={smart_ordering}"
        )
    ...
    async def build_navigation(
        ...
        if orphaned:
            if self.enable_navigation_analytics:
                self.navigation_analytics.orphaned_articles = [
                    a.id for a in orphaned if a.id
                ]
        ...
        self.navigation_tree = NavigationItem(
            title="Root", path="", children=navigation_items, is_section=True
        )
        ...
        if self.navigation_validation:
            validation_report = await self._validate_navigation(
                navigation_items, articles
            )
        ...
        logger.info(
            f"Built enhanced navigation with {len(navigation_items)} top-level items"
        )
        ...
        return {
            ...
            "analytics": self.navigation_analytics.__dict__
            if self.enable_navigation_analytics
            else {},
            "metadata": {
                ...
                "sections_count": len(
                    [
                        ...
                        if item.is_section or item.children
                    ]
                ),
    ...
    def _map_articles_to_categories(
        self, articles: list[Article], categories: list[Category]
        ...
        if self.sort_by_order:
            for category_id in article_map:
                article_map[category_id] = sorted(
                    article_map[category_id], key=lambda a: getattr(a, "order", 0)
                )
    ...
    async def _build_category_navigation(
        ...
        if depth > self.max_depth:
            logger.warning(
                f"Maximum navigation depth reached for category {category.name}"
            )
            return None
        ...
        # Add articles
        for article in articles:
            nav_items.append({article.title: f"{category.slug}/{article.slug}.md"})
    ...
    def _find_home_article(self, articles: list[Article]) -> Article | None:
    ...
    def _nav_item_to_literate(
        self, nav_item: dict[str, Any] | str, level: int = 0
    ) -> list[str]:
    ...
    async def validate_navigation(
        ...
        logger.info(
            f"Navigation validation complete: {len(validation_results['errors'])} errors"
        )
    ...
    async def _apply_smart_ordering(self, articles: list[Article]) -> list[Article]:
        ...
        def calculate_article_weight(article: Article) -> int:
            ...
            if any(
                ...
                pattern in title_lower for pattern in ["tutorial", "guide", "how to"]
            ):
                weight -= 300
            elif any(
                pattern in title_lower for pattern in ["reference", "api", "advanced"]
            ):
                weight += 200
            elif any(
                pattern in title_lower for pattern in ["troubleshoot", "faq", "error"]
            ):
                weight += 400
    ...
    async def _apply_smart_category_ordering(
        ...
        def calculate_category_weight(category: Category) -> int:
            ...
            if any(
                ...
                for pattern in ["getting started", "introduction", "overview"]
            ):
                weight -= 500
            elif any(
                pattern in name_lower for pattern in ["tutorial", "guide", "how to"]
            ):
                weight -= 300
    ...
    async def _generate_section_indexes(
        self, categories: list[Category], articles: list[Article]
    ) -> dict[str, str]:
        ...
        for category in categories:
            category_id = str(category.id)
            category_articles = [
                a for a in articles if str(a.category_id) == category_id
            ]

            if category_articles:
                # Generate index content
                index_content = self._create_section_index_content(
    ...
    def _create_section_index_content(
        ...
        lines.extend(["## Articles in this section", ""])
            ...
            if article.content:
                ...
                if sentences and len(sentences[0]) > 20:
                    description = sentences[0].strip()[:100] + (
                        "..." if len(sentences[0]) > 100 else ""
                    )

            lines.extend(
                [f"### [{article.title}]({article.slug}.md)", f"{description}", ""]
            )
    ...
    def _classify_article_type(self, article: Article) -> str:
        ...
        if any(
            pattern in title_lower for pattern in ["tutorial", "how to", "step by step"]
        ):
            return "tutorial"
        elif any(
            pattern in title_lower for pattern in ["reference", "api", "specification"]
        ):
            return "reference"
        elif any(
            pattern in title_lower for pattern in ["guide", "overview", "introduction"]
        ):
            return "guide"
        elif any(
            pattern in title_lower
            for pattern in ["troubleshoot", "error", "fix", "problem"]
        ):
    ...
    def _generate_breadcrumbs(
        ...
        def build_breadcrumbs(item: NavigationItem, path: list[dict[str, str]] = None):
            ...
            for child in item.children:
                build_breadcrumbs(
                    child, current_path[:-1] if not item.path else current_path
                )
    ...
    async def _validate_navigation(
        self, navigation_items: list[NavigationItem], articles: list[Article]
    ) -> dict[str, Any]:
        ...
        def validate_item(item: NavigationItem, depth: int = 0):
            ...
            if depth > 4:  # More than 4 levels deep
                validation_report["deep_nesting"].append(
                    {"title": item.title, "depth": depth, "path": item.path}
                )
                validation_report["warnings"].append(
                    f"Deep nesting detected: {item.title} at depth {depth}"
                )
        ...
        if duplicates:
            validation_report["warnings"].extend(
                [f"Duplicate title: {title}" for title in duplicates]
            )
        ...
        if missing_article_ids:
            ...
            validation_report["missing_articles"] = [
                {"id": a.id, "title": a.title} for a in missing_articles
            ]
            validation_report["warnings"].extend(
                [f"Article not in navigation: {a.title}" for a in missing_articles]
            )

        # Update analytics
        if self.enable_navigation_analytics:
            self.navigation_analytics.duplicate_titles = duplicates
            self.navigation_analytics.orphaned_articles.extend(
                [a["id"] for a in validation_report["missing_articles"]]
            )
        ...
        analytics.update(
            {
                ...
                "navigation_tree_depth": self._calculate_max_depth(
