...
class ThemeOptimizer:
    ...
    async def optimize(
        ...
        if self.theme == "material":
            await self._optimize_material_theme(
                articles, categories, output_path, results
            )
        elif self.theme == "readthedocs":
            await self._optimize_readthedocs_theme(
                articles, categories, output_path, results
            )
        else:
            await self._optimize_generic_theme(
                articles, categories, output_path, results
            )

        # Apply common optimizations
        await self._apply_common_optimizations(
            articles, categories, output_path, results
        )

        logger.info(
            f"Theme optimization complete: {len(results['optimizations_applied'])} optimizations applied"
        )
        return results
    ...
    async def _create_material_css(
        self, output_path: Path, results: dict[str, Any]
    ) -> None:
        """Create custom CSS for Material theme."""
...
"""
    ...
    async def _setup_material_social_cards(
        self, output_path: Path, results: dict[str, Any]
    ) -> None:
    ...
    async def _create_material_templates(
        self, output_path: Path, results: dict[str, Any]
    ) -> None:
    ...
    async def _optimize_material_content(
        self, articles: list[Article], output_path: Path, results: dict[str, Any]
    ) -> None:
        """Optimize content for Material theme features."""
...
"""
    ...
    def get_theme_config_updates(self) -> dict[str, Any]:
        ...
        if self.custom_js:
            ...
            for js_file in self.custom_js:
                config_updates["extra_javascript"].append(
                    f"javascripts/{Path(js_file).name}"
                )
