...
class MkDocsYamlLoader(yaml.SafeLoader):
    """SafeLoader that tolerates MkDocs/Material ``!!python/name`` style tags.

    MkDocs Material configs legitimately use tags such as
    ``!!python/name:material.extensions.emoji.to_svg`` which the standard
    SafeLoader cannot construct. These are only meaningful at MkDocs runtime, so
    for validation we resolve them to an opaque placeholder string.
    """


def _construct_python_tag(loader: yaml.Loader, tag_suffix: str, node: yaml.Node) -> str:
...
for _prefix in (
    ...
    MkDocsYamlLoader.add_multi_constructor(_prefix, _construct_python_tag)


def load_mkdocs_yaml(content: str) -> Any:
    """Parse MkDocs YAML tolerating Material python tags (safe otherwise)."""
    return yaml.load(content, Loader=MkDocsYamlLoader)
...
class ConfigGenerator:
    ...
    async def generate_config_from_template(
        ...
        try:
            ...
            error_msg = (
                f"Unexpected error generating config from template {template_name}: {e}"
            )
            logger.error(error_msg)
            raise Document360Error(
                message=error_msg,
    ...
    def _validate_generated_config(
        self, config_content: str, template_name: str
    ) -> None:
        ...
        """
        try:
            # Parse YAML to ensure it's valid (tolerating Material python tags)
            ...
            logger.debug(
                f"Configuration validation passed for template: {template_name}"
            )
    ...
    def _validate_mkdocs_structure(
        self, config: dict[str, Any], template_name: str
    ) -> None:
        ...
        if not isinstance(config["site_name"], str):
            error_msg = (
                f"Invalid site_name type in template '{template_name}': expected string"
            )
            logger.error(error_msg)
            raise Document360Error(
                message=error_msg,
    ...
    def _validate_theme_config(
        self, theme_config: str | dict[str, Any], template_name: str
    ) -> None:
        ...
        if theme_name not in known_themes:
            logger.warning(
                f"Unknown theme '{theme_name}' in template '{template_name}'. Known themes: {known_themes}"
            )
        ...
        try:
            ...
            if "theme" in config:
                ...
                if isinstance(theme, dict) and "name" not in theme:
                    results["warnings"].append(
                        "Theme configuration should include 'name' field"
                    )
                elif isinstance(theme, str) and theme not in [
                    "material",
                    "readthedocs",
                    "mkdocs",
                ]:
            ...
            if "nav" not in config:
                results["suggestions"].append(
                    "Consider adding navigation structure for better user experience"
                )

            # Check for SEO fields
            if "site_description" not in config:
                results["suggestions"].append(
                    "Consider adding site_description for better SEO"
                )
            ...
            if "search" not in plugin_names:
                results["warnings"].append(
                    "Search plugin not found - users won't be able to search content"
                )

            if (
                "minify" not in plugin_names
                and config.get("theme", {}).get("name") == "material"
            ):
                results["suggestions"].append(
                    "Consider adding minify plugin for better performance"
                )

            logger.info(
                f"Configuration validation completed. Valid: {results['valid']}, Errors: {len(results['errors'])}, Warnings: {len(results['warnings'])}"
            )
