...
class AccessibilityOptimizer:
    ...
    def __init__(
        ...
        logger.info(
            f"Initialized AccessibilityOptimizer for WCAG {wcag_level.value} compliance"
        )
    ...
    async def enhance_content(
        ...
        logger.info(
            f"Found {report.total_issues} issues ({len(report.errors)} errors, {len(report.warnings)} warnings)"
        )
    ...
    def _enhance_links(
        self, content: str, report: AccessibilityReport, base_url: str | None
    ) -> str:
        ...
        def enhance_link(match):
            ...
            if url in link_text or link_text.startswith(
                ("http://", "https://", "www.")
            ):
                report.add_issue(
                    AccessibilityIssue(
    ...
    def _validate_content_structure(
        self, content: str, report: AccessibilityReport
    ) -> None:
    ...
    async def audit_full_site(
        ...
        for page_info in pages:
            try:
                ...
                logger.error(
                    f"Error auditing page {page_info.get('title', 'Unknown')}: {e}"
                )
                continue
        ...
        if site_report["pages_audited"] > 0:
            compliance_rate = (
                site_report["summary"]["compliant_pages"] / site_report["pages_audited"]
            ) * 100
            site_report["compliance_percentage"] = compliance_rate

        logger.info(
            f"Site accessibility audit completed: {site_report['pages_audited']} pages audited"
        )
        logger.info(
            f"Compliance rate: {site_report.get('compliance_percentage', 0):.1f}%"
        )
