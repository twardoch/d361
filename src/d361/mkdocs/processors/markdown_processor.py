...
class MarkdownProcessor:
    ...
    def _convert_d360_callouts(self, content: str) -> str:
        ...
        for pattern, admonition_type in callout_patterns:
            ...
            for match in matches:
                ...
                admonition = f"\n!!! {admonition_type}\n\n    {admonition_content}\n\n"
                content = re.sub(
                    pattern, admonition, content, flags=re.DOTALL | re.IGNORECASE
                )
        ...
        for pattern in code_patterns:
            ...
            for match in matches:
                ...
                content = content.replace(
                    f"<pre><code{language if 'language' in pattern else ''}>{code_content}</code></pre>",
                    formatted_code,
                )
    ...
    def _convert_d360_tables(self, content: str) -> str:
        ...
        for table_content in tables:
            try:
                ...
                content = content.replace(
                    f"<table{table_content}</table>", markdown_table
                )
    ...
    async def _html_to_markdown(self, content: str) -> str:
        ...
        content = re.sub(
            r"<em[^>]*>(.*?)</em>", r"*\1*", content, flags=re.DOTALL | re.IGNORECASE
        )
        ...
        content = re.sub(
            link_pattern, r"[\2](\1)", content, flags=re.DOTALL | re.IGNORECASE
        )
