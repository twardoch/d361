...
try:
    ...
    logger.warning(
        "HTML processing dependencies not available - install html2text, beautifulsoup4, requests for full functionality"
    )
...
try:
    ...
    logger.warning(
        "Python Markdown extensions not available - install markdown for enhanced processing"
    )
...
class ContentEnhancer:
    ...
    def __init__(
        ...
        custom_css_classes: dict[str, str] | None = None,
        # Phase 2 enhancements
        enable_html_conversion: bool = True,
        mkdocs_extensions: list[str] | None = None,
        ...
        # Phase 2 enhancements
        self.enable_html_conversion = (
            enable_html_conversion and HTML_PROCESSING_AVAILABLE
        )
        self.mkdocs_extensions = mkdocs_extensions or [
            "admonition",
            "codehilite",
            "tables",
            "toc",
        ...
        self._admonition_pattern = re.compile(
            r"^!!! (note|warning|info|tip|success|question|failure|danger|bug|example|quote)\s*(.*?)$",
            re.MULTILINE,
        )

        logger.debug(
            f"Initialized ContentEnhancer with site_url={site_url}, HTML conversion={self.enable_html_conversion}"
        )
    ...
    async def enhance_article(self, article: Article) -> dict[str, Any]:
        ...
        result = {
            ...
            "edit_url": self._generate_edit_url(article)
            if self.add_edit_links
            else None,
            # Phase 2 additions
        ...
        logger.debug(
            f"Enhanced article '{article.title}' with quality score: {quality_metrics.get('overall_score', 0)}, reading time: {reading_time}min"
        )
        return result
    ...
    def _add_css_classes(self, content: str) -> str:
        ...
        for element, css_class in self.custom_css_classes.items():
            if element == "tables":
                content = re.sub(r"(\|.*\|)", f"\\1{{: .{css_class} }}", content)
    ...
    def _convert_to_admonitions(self, content: str) -> str:
        ...
        content = re.sub(
            ...
            lambda m: (
                f'!!! {m.group(1)} "{m.group(2) or m.group(1).title()}"\n    {m.group(3).strip()}'
            ),
            content,
            flags=re.MULTILINE | re.DOTALL,
        )
    ...
    def _enhance_code_blocks_superfences(self, content: str) -> str:
        ...
        def enhance_code_block(match):
            ...
            if language in ["bash", "shell", "sh"]:
                if any(
                    keyword in code
                    for keyword in ["curl", "wget", "pip install", "npm install"]
                ):
        ...
        for pattern, tab_type in tab_patterns:
            ...
            def replace_with_tabs(match):
                ...
                tab_names = (
                    ...
                    if tab_type == "code-examples"
                    else ["Windows", "macOS", "Linux"]
                    if tab_type == "platform-specific"
                    else [f"Step {i}" for i in range(1, len(parts) + 1)]
                )
            ...
            content = re.sub(
                pattern, replace_with_tabs, content, flags=re.MULTILINE | re.DOTALL
            )
    ...
    def _convert_to_task_lists(self, content: str) -> str:
        ...
        content = re.sub(
            r"^(\s*)- (.*?)(\s*)\(pending\)", r"\1- [ ] \2", content, flags=re.MULTILINE
        )
        content = re.sub(
            r"^(\s*)- (.*?)(\s*)\(todo\)", r"\1- [ ] \2", content, flags=re.MULTILINE
        )

        return content

    async def _assess_quality_advanced(
        self, content: str, article: Article
    ) -> dict[str, Any]:
        ...
        enhanced_score = (
            ...
            + (
                advanced_metrics["heading_structure_score"]
                * weights["heading_structure"]
            )
            ...
            + (
                advanced_metrics["mkdocs_compatibility_score"]
                * weights["mkdocs_compatibility"]
            )
        )
        ...
        combined_metrics["quality_level"] = self._determine_quality_level(
            enhanced_score
        )
        combined_metrics["recommendations"] = self._generate_quality_recommendations(
            combined_metrics
        )
    ...
    def _calculate_reading_time(self, content: str) -> int:
        ...
        code_adjustment = (code_lines * 0.5) + (
            code_blocks * 1
        )  # 0.5 min per code line + 1 min per block
    ...
    def _analyze_internal_linking(self, content: str) -> int:
        ...
        internal_links = [
            link for link in links if not link[1].startswith(("http://", "https://"))
        ]
    ...
    def _analyze_seo_optimization(self, content: str, article: Article) -> int:
        ...
        internal_links = [
            link
            for link in self._link_pattern.findall(content)
    ...
    def _generate_quality_recommendations(self, metrics: dict[str, Any]) -> list[str]:
        ...
        if metrics.get("word_count", 0) < 100:
            recommendations.append(
                "Consider expanding the content - articles with at least 200 words tend to be more helpful"
            )

        if metrics.get("heading_count", 0) == 0:
            recommendations.append(
                "Add headings to improve content structure and navigation"
            )

        if not metrics.get("has_introduction", False):
            recommendations.append(
                "Add an introduction to help readers understand what they'll learn"
            )
        ...
        if metrics.get("internal_linking_score", 0) < 40:
            recommendations.append(
                "Add more internal links to related articles to improve navigation"
            )

        if metrics.get("media_richness_score", 0) < 40:
            recommendations.append(
                "Consider adding relevant images or diagrams to illustrate key concepts"
            )

        if metrics.get("readability_score", 0) < 60:
            recommendations.append(
                "Consider breaking up long sentences and paragraphs for better readability"
            )
    ...
    def _determine_template(self, article: Article) -> str | None:
    ...
    def _is_internal_d360_link(self, url: str) -> bool:
        ...
        return (
            "document360" in url.lower() or url.startswith("/") or url.startswith("../")
        )
    ...
    def _generate_edit_url(self, article: Article) -> str | None:
