...
class PluginConfig:
    ...
    theme_compatibility: list[str] = field(
        default_factory=lambda: ["material", "readthedocs", "mkdocs"]
    )
    description: str = ""
...
class PluginSet:
    ...
    required_for_theme: str | None = None
...
class PluginManager:
        ...
        plugins = [
            PluginConfig(
                ...
                config={
                    ...
                    "cache_safe": True,
                    "js_files": ["assets/javascripts/bundle.*.min.js"],
                    "css_files": ["assets/stylesheets/*.min.css"],
                },
    ...
    async def get_optimal_plugins(
        ...
        custom_plugins: list[PluginConfig] | None = None,
    ) -> list[dict[str, Any]]:
            ...
            for plugin_config in plugin_set.plugins:
                if plugin_config.enabled and plugin_config.name not in plugin_names:
                    ...
                    if self.theme in plugin_config.theme_compatibility:
                        ...
                        logger.debug(
                            f"Skipping plugin {plugin_config.name} (incompatible with {self.theme})"
                        )
    ...
    async def get_plugin_dependencies(
        ...
        """
        return self._collect_plugin_dependencies(
    ...
    def _collect_plugin_dependencies(
        ...
        """Synchronous core of dependency collection (no I/O, no awaits)."""
        ...
        for plugin_set in plugin_sets:
            for plugin_config in plugin_set.plugins:
                if plugin_config.enabled and plugin_config.required_dependencies:
                    dependencies[plugin_config.name] = (
                        plugin_config.required_dependencies
                    )
    ...
    def get_installation_guide(
        ...
        """
        # Collect dependencies synchronously (safe to call inside a running loop)
        dependencies = self._collect_plugin_dependencies(
            enable_seo=enable_seo,
            enable_performance=enable_performance,
            enable_accessibility=enable_accessibility,
            enable_advanced=enable_advanced,
    ...
    def validate_plugin_configuration(
        ...
        if "search" not in plugin_names:
            results["warnings"].append(
                "Search plugin missing - users won't be able to search content"
            )
        ...
        if self.theme == "material":
            if "social" not in plugin_names:
                results["suggestions"].append(
                    "Consider adding social plugin for Material theme social cards"
                )

            if "offline" not in plugin_names and self.enable_offline:
                results["suggestions"].append(
                    "Consider adding offline plugin for offline functionality"
                )
            ...
            extensions.append(
                {
                    "pymdownx.emoji": {
                        ...
                        "options": {"custom_icons": ["material/extensions/emoji.json"]},
                    }
                }
            )
            ...
            for ext in extensions:
                if isinstance(ext, dict) and "pymdownx.superfences" in ext:
                    ...
                    ext["pymdownx.superfences"]["custom_fences"] = [
                        fence
                        for fence in fences
                        if fence.get("name") not in ["mermaid"]
                    ]

        logger.info(
            f"Built Markdown extensions configuration with {len(extensions)} extensions"
        )
        return extensions
