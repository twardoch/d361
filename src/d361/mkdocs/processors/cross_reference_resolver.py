...
try:
    ...
    logger.warning(
        "HTTP validation dependencies not available - install httpx for full link validation"
    )
...
class CrossReferenceResolver:
    ...
    def __init__(
        ...
        autorefs_plugin_config: dict[str, Any] | None = None,
        ...
        self.enable_http_validation = (
            enable_http_validation and HTTP_VALIDATION_AVAILABLE
        )
        ...
        self._reference_index: dict[
            str, list[str]
        ] = {}  # Article ID -> list of referencing articles
        ...
        self._d360_link_pattern = re.compile(
            r"https?://[^/]*document360[^/]*/[^/]+/(\w+)(?:/(\w+))?"
        )
        ...
        logger.debug(
            f"Initialized CrossReferenceResolver with {len(articles)} articles, HTTP validation={self.enable_http_validation}, autorefs plugin={self.enable_autorefs_plugin}"
        )

    async def resolve_references(
        self, content: str, current_article: Article
    ) -> dict[str, Any]:
        ...
        if self.enable_http_validation:
            external_links = [
                link
                for link in links
                if link.original_url.startswith(("http://", "https://"))
            ]
        ...
        if self.enable_autorefs_plugin and autorefs:
            resolved_content = self._process_autorefs(
                resolved_content, autorefs, current_article
            )
        ...
        if self.smart_link_suggestions:
            smart_suggestions = await self._generate_smart_suggestions(
        ...
        return {
            ...
            "external_links": [
                link for link in resolved_links if link.link_type == "external"
            ],
            "internal_links": [
                link for link in resolved_links if link.link_type == "internal"
            ],
            # Phase 2 additions
            "autorefs": autorefs,
    ...
    async def resolve_references_bulk(
        self, articles: list[Article], navigation: dict[str, Any]
    ) -> list[Article]:
        ...
        for article in articles:
            try:
                ...
                resolution_result = await self.resolve_references(
                    article.content or "", article
                )
                ...
                logger.error(
                    f"Error resolving references for article {article.id}: {e}"
                )
    ...
    def generate_link_report(self) -> dict[str, Any]:
        ...
        """
        total_links = sum(
            len(article_anchors) for article_anchors in self._anchor_index.values()
        )
        ...
        return {
            ...
            "validation_summary": {
                "total_processed": len(
                    [link for links in self._anchor_index.values() for link in links]
                ),
                    ...
                    article_suggestions.append(
                        {
                            ...
                            / max(len(article_keywords), len(other_keywords)),
                        }
                    )
    ...
    async def _resolve_link(
        self, link: LinkReference, current_article: Article
    ) -> LinkReference:
    ...
    def _apply_link_resolutions(self, content: str, links: list[LinkReference]) -> str:
        ...
        for match in self._link_pattern.finditer(content):
            ...
            resolved_link = next(
                (link for link in links if link.original_url == original_url), None
            )
    ...
    def _is_document360_link(self, url: str) -> bool:
        ...
        return (
            "document360" in url.lower()
            or self._d360_link_pattern.match(url) is not None
        )
    ...
    def _extract_keywords(self, title: str) -> list[str]:
        ...
        return [
            word
            for word in words
            if word not in {"the", "and", "for", "with", "from", "how", "what", "why"}
        ]
    ...
    async def _validate_external_links_batch(
        self, external_links: list[LinkReference]
    ) -> None:
    ...
    def _process_autorefs(
        self, content: str, autorefs: list[dict[str, Any]], current_article: Article
    ) -> str:
    ...
    def _resolve_autoref_identifier(self, identifier: str) -> str | None:
        ...
        if identifier in self._category_index:
            ...
            category_articles = [
                ...
                if getattr(a, "category_id", None) == category.id
            ]
    ...
    def _update_reference_index(
        self, resolved_links: list[LinkReference], current_article: Article
    ) -> None:
    ...
    async def _generate_smart_suggestions(
        self, current_article: Article, content: str
    ) -> list[dict[str, Any]]:
                ...
                suggestions.append(
                    {
                        ...
                        "explanation": self._generate_suggestion_explanation(
                            current_article, article, relevance_score
                        ),
                    }
                )
                    ...
                    suggestions.append(
                        {
                            ...
                            "explanation": "Related article in the same category",
                        }
                    )
        ...
        # Cache the results
        self._suggestion_cache[current_article.id] = suggestions[
            :10
        ]  # Top 10 suggestions
    ...
    def _get_validation_summary(self, links: list[LinkReference]) -> dict[str, Any]:
        ...
        return {
            ...
            "validation_rate": (valid_links / total_links * 100)
            if total_links > 0
            else 100,
    ...
    def _get_analytics_summary(self, links: list[LinkReference]) -> dict[str, Any]:
        ...
        return {
            ...
            "broken_url_count": sum(
                1 for data in self._link_analytics.values() if data["is_broken"]
            ),
