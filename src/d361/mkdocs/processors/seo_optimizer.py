...
class SEOOptimizer:
    ...
    def __init__(
        ...
        # Parse site URL for domain info (use the normalized, slash-stripped
        # value so parsed_url.path has no trailing slash)
        self.parsed_url = urlparse(self.site_url)
    ...
    def _generate_description(
        self, content: str, title: str, max_length: int = 155
    ) -> str:
    ...
    async def generate_structured_data(
        ...
        publisher = {"@type": "Organization", "name": self.organization_name}

        if self.organization_logo:
            publisher["logo"] = {"@type": "ImageObject", "url": self.organization_logo}
    ...
    def validate_seo_config(self, config: dict[str, Any]) -> dict[str, Any]:
        ...
        if "site_url" not in config:
            results["errors"].append(
                "Missing site_url - required for proper canonical URLs"
            )
            results["valid"] = False
        ...
        if isinstance(theme, dict):
            ...
            if "social" not in config.get("extra", {}):
                results["suggestions"].append(
                    "Consider adding social media links for better engagement"
                )

            # Check for analytics
            if "analytics" not in config.get("extra", {}):
                results["suggestions"].append(
                    "Consider adding analytics tracking for insights"
                )
        ...
        if "social" not in plugin_names and theme.get("name") == "material":
            results["suggestions"].append(
                "Consider enabling social cards plugin for better sharing"
            )

        if "redirects" not in plugin_names:
            results["suggestions"].append(
                "Consider adding redirects plugin for URL management"
            )
        ...
        if "meta" not in extension_names:
            results["warnings"].append(
                "Missing 'meta' extension - needed for page-specific SEO metadata"
            )
    ...
    async def generate_sitemap_urls(
        ...
        changefreq_map: dict[str, str] | None = None,
    ) -> list[dict[str, Any]]:
        ...
        # Caller-supplied maps take precedence over the defaults.
        final_priority_map = {**default_priority_map, **(priority_map or {})}
        final_changefreq_map = {**default_changefreq_map, **(changefreq_map or {})}

        def _match(url_path: str, mapping: dict, default):
            """Return the value for the most specific matching pattern.

            The root "/" only matches the exact home path; other patterns match
            as path prefixes, and the longest (most specific) prefix wins so that
            e.g. "/api" beats a generic entry.
            """
            best_pattern = None
            for pattern in mapping:
                matches = (
                    url_path == pattern
                    if pattern == "/"
                    else url_path.startswith(pattern)
                )
                if matches and (
                    best_pattern is None or len(pattern) > len(best_pattern)
                ):
                    best_pattern = pattern
            return mapping[best_pattern] if best_pattern is not None else default

        for page in pages:
            url_path = page.get("url", "")
            full_url = urljoin(self.site_url, url_path)

            # Determine priority and change frequency by most-specific match
            priority = _match(url_path, final_priority_map, 0.5)
            changefreq = _match(url_path, final_changefreq_map, "monthly")
