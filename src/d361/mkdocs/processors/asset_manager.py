...
try:
    ...
    logger.warning(
        "Social cards dependencies not available - some PIL features may be limited"
    )
...
class AssetManager:
    ...
    def __init__(
        ...
        social_card_font_family: str | None = None,
        social_card_logo_path: str | None = None,
        ...
        # Public attributes reflect the caller's intent; actual imaging work is
        # guarded at the point of use by IMAGING_AVAILABLE/SOCIAL_CARDS_AVAILABLE
        # so a missing PIL install degrades gracefully instead of silently
        # rewriting the configuration.
        self.enable_optimization = enable_optimization
        self.generate_responsive = generate_responsive
        ...
        # Phase 2 enhancements
        self.generate_social_cards = generate_social_cards
        ...
        self.social_cards_dir = (
            self.output_dir / "social_cards" if self.generate_social_cards else None
        )
        ...
        self.social_cards_generated: dict[
            str, str
        ] = {}  # article_id -> social_card_path
        self.social_cards_cache: dict[
            str, dict[str, Any]
        ] = {}  # Cache for social card metadata
        ...
        logger.debug(
            f"Initialized AssetManager with output_dir={output_dir}, optimization={enable_optimization}"
        )

    async def process_assets(
        self, content: str, article_context: dict | None = None
    ) -> list[AssetReference]:
        ...
        if not self._http_client and (
            self.download_external_assets or self.enable_optimization
        ):
            self._http_client = httpx.AsyncClient(
                timeout=30.0, headers={"User-Agent": "d361-mkdocs-exporter/1.0"}
            )
        ...
        document_assets = await self._extract_and_process_documents(
        ...
        if self.generate_social_cards and article_context:
            social_card_asset = await self._generate_social_card(
        ...
        logger.debug(
            f"Processed {len(assets)} assets ({len(image_assets)} images, {len(document_assets)} documents{', 1 social card' if social_card_asset else ''})"
        )
        return assets

    def update_asset_references(
        self, content: str, assets: list[AssetReference]
    ) -> str:
    ...
    async def _extract_and_process_images(
        ...
        for match in self._image_pattern.finditer(content):
            ...
            try:
                # Process the image
                processed_asset = await self._process_single_image(
                    asset, article_context
                )
    ...
    async def _extract_and_process_documents(
        self, content: str, article_context: dict | None = None
    ) -> list[AssetReference]:
        ...
        for match in self._link_pattern.finditer(content):
            ...
            try:
                ...
                processed_asset = await self._process_single_document(
                    asset, article_context
                )
    ...
    async def _process_single_image(
        self, asset: AssetReference, article_context: dict | None = None
    ) -> AssetReference:
        ...
        if (
            self.download_external_assets and asset.is_external
        ) or self.enable_optimization:
    ...
    async def _download_asset(
        self, asset: AssetReference, target_dir: Path
    ) -> Path | None:
        ...
        try:
            ...
            filename = self._generate_filename(
                asset.original_url, response.headers.get("content-type")
            )
    ...
    async def _optimize_image(
        self, image_path: Path, asset: AssetReference
    ) -> Path | None:
        """Optimize image file."""
            ...
            with Image.open(image_path) as img:
                ...
                if img.width > self.max_image_width:
                    ...
                    optimized_img = optimized_img.resize(
                        (self.max_image_width, new_height), Image.Resampling.LANCZOS
                    )
                ...
                logger.debug(
                    f"Optimized image: {image_path} -> {output_path} ({original_size} -> {optimized_size} bytes)"
                )
                return output_path
        ...
        try:
            ...
            with Image.open(image_path) as img:
                for breakpoint in breakpoints:
                    ...
                    new_height = int(img.height * ratio)
                    variant_img = img.resize(
                        (breakpoint, new_height), Image.Resampling.LANCZOS
                    )
            ...
            logger.error(
                f"Failed to generate responsive variants for {image_path}: {e}"
            )
            return []
    ...
    def _generate_filename(self, url: str, content_type: str | None = None) -> str:
    ...
    def _is_document360_url(self, url: str) -> bool:
        ...
        return "document360" in url.lower() and (
            "cdn" in url.lower() or "assets" in url.lower()
        )
    ...
    def get_asset_report(self) -> dict[str, Any]:
        ...
        optimized_assets = sum(
            1 for asset in self.processed_assets.values() if asset.optimization_applied
        )
        ...
        return {
            "summary": {
                ...
                "average_compression_ratio": sum(
                    r.compression_ratio for r in self.optimization_stats.values()
                )
                / len(self.optimization_stats)
            ...
            "asset_types": {
                asset_type: len(
                    [
                        a
                        for a in self.processed_assets.values()
                        if a.asset_type == asset_type
                    ]
                )
                for asset_type in set(
                    asset.asset_type for asset in self.processed_assets.values()
                )
            },
        }
    ...
    async def _generate_social_card(
        self, article_context: dict[str, Any], content: str
    ) -> AssetReference | None:
        ...
        try:
            ...
            article_description = self._extract_social_description(
                content, article_context
            )
            ...
            logger.error(
                f"Failed to generate social card for article '{article_context.get('title', 'Unknown')}': {e}"
            )

        return None

    def _extract_social_description(
        self, content: str, article_context: dict[str, Any]
    ) -> str:
        ...
        if article_context.get("description"):
        ...
        try:
            ...
            if self.social_card_logo_path and Path(self.social_card_logo_path).exists():
                logo_height = self._draw_logo(
                    draw,
                    Path(self.social_card_logo_path),
                    padding,
                    padding,
                    max_height=80,
                )
            ...
            for line in title_lines:
                draw.text(
                    (padding, current_y),
                    line,
                    fill=self.social_card_text_color,
                    font=title_font,
                )
            ...
            description_lines = self._wrap_text(
                description, content_width, description_font, draw
            )

            for line in description_lines[:3]:  # Max 3 lines for description
                draw.text(
                    (padding, description_y),
                    line,
                    fill=self._lighten_color(self.social_card_text_color),
            ...
            draw.text(
                (padding, brand_y),
                brand_text,
                fill=self.social_card_accent_color,
                font=brand_font,
            )
            ...
            line_y = brand_y - 20
            draw.rectangle(
                [padding, line_y, padding + 100, line_y + line_width],
                fill=self.social_card_accent_color,
            )
    ...
    def _load_font(self, size: int) -> "ImageFont.FreeTypeFont":  # type: ignore[name-defined]
            ...
            "generated_cards": [
                {
                    ...
                    "path": path,
                    "url": self._generate_asset_url(Path(path))
                    if Path(path).exists()
                    else None,
                }
