...
class ArchiveParserConfig(BaseModel):
    ...
    extract_text_content: bool = Field(
        default=True, description="Extract text content from files"
    )

    extract_metadata: bool = Field(
        default=True, description="Extract metadata from files"
    )
    ...
    enable_compression: bool = Field(
        default=True, description="Enable content compression for storage"
    )

    batch_size: int = Field(
        default=100, ge=1, le=1000, description="Batch size for processing entries"
    )

    # Validation settings
    validate_json: bool = Field(
        default=True, description="Validate JSON content structure"
    )

    strict_validation: bool = Field(
        default=False, description="Enable strict validation of content structure"
    )
...
class ArchiveMetadata(BaseModel):
    ...
    processed_entries: int = Field(
        default=0, description="Successfully processed entries"
    )
    ...
    parsed_at: datetime = Field(
        default_factory=datetime.now, description="When archive was parsed"
    )
    ...
    root_directories: list[str] = Field(
        default_factory=list, description="Root level directories"
    )
    file_extensions: dict[str, int] = Field(
        default_factory=dict, description="File extensions and counts"
    )
...
class ParsedArchive(BaseModel):
    ...
    entries: list[ArchiveEntry] = Field(
        default_factory=list, description="Parsed archive entries"
    )
    articles: list[Article] = Field(
        default_factory=list, description="Extracted articles"
    )
    categories: list[Category] = Field(
        default_factory=list, description="Extracted categories"
    )
    projects: list[ProjectVersion] = Field(
        default_factory=list, description="Extracted projects"
    )
...
class ArchiveParser:
    ...
    def __init__(self, config: ArchiveParserConfig | None = None):
        """
        ...
        logger.info(
            "ArchiveParser initialized",
            supported_formats=list(self.config.supported_formats),
            max_size=self.config.max_archive_size,
        )

    async def parse_archive(self, archive_path: str | Path) -> ParsedArchive:
        """
        ...
        try:
            ...
            entries = await self._parse_archive_contents(
                archive_path, archive_format, metadata
            )

            # Extract structured data
            articles, categories, projects = await self._extract_structured_data(
                entries
            )
            ...
            logger.info(
                "Archive parsing completed successfully",
                entries=len(entries),
            ...
            with tarfile.open(archive_path, "r:*") as tar_file:
                ...
                for tar_info in members:
                    ...
                    if tar_info.size > self.config.max_file_size:
                        logger.warning(
                            f"Skipping large file: {tar_info.name} ({tar_info.size} bytes)"
                        )
                        continue
    ...
    async def _decode_content(self, content: bytes) -> str | None:
    ...
    def _determine_content_type(
        self, path: str, text_content: str | None
    ) -> ContentType:
    ...
    async def _extract_article_metadata(
        self, entry: ArchiveEntry, content: str
    ) -> None:
    ...
    async def _extract_structured_data(
        ...
        for entry in entries:
            try:
                ...
                logger.warning(
                    f"Failed to create structured data from {entry.path}: {e}"
                )

        return articles, categories, projects

    async def _create_article_from_entry(self, entry: ArchiveEntry) -> Article | None:
    ...
    async def _create_category_from_entry(self, entry: ArchiveEntry) -> Category | None:
    ...
    async def _create_project_from_entry(
