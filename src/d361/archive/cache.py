...
class CacheConfig(BaseModel):
    ...
    schema_config: ArchiveSchema = Field(
        default_factory=ArchiveSchema, description="Database schema configuration"
    )
    ...
    max_size_mb: int = Field(
        default=500, ge=1, description="Maximum cache size in megabytes"
    )

    max_entries: int = Field(
        default=10000, ge=1, description="Maximum number of cache entries"
    )
    ...
    enable_background_cleanup: bool = Field(
        default=True, description="Enable background cleanup task"
    )
...
class ConnectionPool:
    ...
    def __init__(self, db_path: Path, max_connections: int = 10):
        ...
        logger.debug(
            f"ConnectionPool initialized with {max_connections} max connections"
        )
...
class SqliteCache:
    ...
    def __init__(self, config: CacheConfig | None = None):
        ...
        self.connection_pool = ConnectionPool(
            self.config.db_path, self.config.max_connections
        )
    ...
    async def _ensure_cache_schema(self) -> None:
        ...
        try:
            ...
            try:
                ...
                cursor.execute(
                    "CREATE INDEX IF NOT EXISTS idx_cache_ttl ON cache_entries (ttl_seconds)"
                )
    ...
    async def _ensure_cache_capacity(
        self, cursor: sqlite3.Cursor, new_entry_size: int
    ) -> None:
        ...
        needs_eviction = (
            self.stats.total_size + new_entry_size
            > self.config.max_size_mb * 1024 * 1024
            or self.stats.entry_count >= self.config.max_entries
        )
    ...
    def _deserialize_value(self, data: str | bytes) -> Any:
