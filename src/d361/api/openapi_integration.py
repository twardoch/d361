...
class OpenApiConfig(BaseModel):
    ...
    auto_update: bool = Field(
        default=True, description="Automatically check for spec updates"
    )

    update_check_interval: int = Field(
        default=3600, ge=60, le=86400, description="Seconds between update checks"
    )
    ...
    generate_models: bool = Field(
        default=False, description="Automatically generate Pydantic models"
    )

    # Validation settings
    strict_validation: bool = Field(
        default=True, description="Enable strict OpenAPI validation"
    )
...
class OpenApiSpec(BaseModel):
    ...
    fetched_at: datetime = Field(
        default_factory=datetime.now, description="When spec was fetched"
    )
    etag: str | None = Field(None, description="HTTP ETag for caching")
...
class OpenApiIntegration:
    ...
    def __init__(
        ...
        # Internal state
        self._current_spec: OpenApiSpec | None = None
        self._last_check: datetime | None = None

        logger.info(
            "OpenApiIntegration initialized",
    ...
    async def get_spec(self, force_refresh: bool = False) -> OpenApiSpec:
        ...
        if (
            not force_refresh
            and self._current_spec
            and not self._current_spec.is_expired(self.config.cache_ttl_hours)
        ):
    ...
    async def _load_cached_spec(self) -> OpenApiSpec | None:
        ...
        try:
            ...
            spec = OpenApiSpec(
                ...
                last_modified=datetime.fromisoformat(cached_data["last_modified"])
    ...
    async def get_endpoint_changes(
        self, old_spec: OpenApiSpec, new_spec: OpenApiSpec
    ) -> dict[str, Any]:
        """
...
'''
    ...
    async def get_api_summary(self) -> dict[str, Any]:
        ...
        return {
            ...
            "endpoints_by_tag": {
                tag: len(endpoints) for tag, endpoints in by_tags.items()
            },
            "available_methods": list(
                set(endpoint["method"] for endpoint in endpoints)
            ),
