...
class ApiError(Document360Error):
    ...
    def __init__(self, message: str, response: HttpResponse | None = None, **kwargs):
        super().__init__(
            message,
            response=response,
...
class NotFoundError(Document360Error):
    ...
    def __init__(self, message: str, resource_id: str | None = None, **kwargs):
        super().__init__(
            message,
...
class ErrorHandler:
    ...
    @staticmethod
    def classify_error(
        response: HttpResponse | None, exception: Exception | None = None
    ) -> Document360Error:
        """
        ...
        if response:
            ...
            if response.json_data and isinstance(response.json_data, dict):
                error_detail = response.json_data.get(
                    "error", response.json_data.get("message")
                )
                if error_detail:
            ...
            if status_code == 401:
                ...
                return NotFoundError(
                    "Resource not found", resource_id=resource_id, response=response
                )
                ...
                return ValidationError(
                    "Validation failed", field=field, value=value, response=response
                )
                ...
                return RateLimitError(
                    "Rate limit exceeded", response=response, retry_after=retry_after
                )
    ...
    def log_error(error: Document360Error, operation: str, attempt: int = 1) -> None:
        ...
        if error.severity == ErrorSeverity.CRITICAL:
            ...
            logger.warning(
                f"High severity error in {operation}: {error}", **log_context
            )
        elif error.severity == ErrorSeverity.MEDIUM:
            logger.warning(
                f"Medium severity error in {operation}: {error}", **log_context
            )
        else:
