...
class TokenStats:
    ...
    def update_rate_limit(
        self, calls_remaining: int, reset_time: datetime | None = None
    ) -> None:
        """
    ...
    def can_make_call(self) -> bool:
        ...
        return (
            self.health not in [TokenHealth.EXPIRED, TokenHealth.ERROR]
    ...
    # Request-oriented aliases over the underlying call/error counters.
    @property
    def total_requests(self) -> int:
        """Total recorded requests for this token."""
        return self.total_calls

    @property
    def failed_requests(self) -> int:
        """Number of failed requests for this token."""
        return self.error_count

    @property
    def successful_requests(self) -> int:
        """Number of successful requests for this token."""
        return self.total_calls - self.error_count

    @property
    def average_response_time(self) -> float:
        """Average response time across recorded calls."""
...
class RateLimiter:
    ...
    def __init__(
        ...
        Args:
            ...
            requests_per_minute: Alias for ``calls_per_minute`` (takes precedence)
        """
        if requests_per_minute is not None:
            calls_per_minute = requests_per_minute
    ...
    async def can_proceed(self, token: str | None = None) -> bool:
        """Whether another call may proceed under the per-minute limit.

        The ``token`` argument is accepted for a token-aware call signature but
        this limiter tracks a single rolling window.
        """
        now = datetime.now()
        cutoff = now - timedelta(minutes=1)
        recent = [ts for ts in self.call_timestamps if ts > cutoff]
        return len(recent) < self.calls_per_minute

    async def record_request(self, token: str | None = None) -> None:
        """Record that a request was made (async, token-agnostic wrapper)."""
        self.record_call()
...
class TokenManager:
    ...
    async def get_best_token(self) -> str | None:
        """
        ...
        """
        available_tokens = [
            token for token, stats in self.tokens.items() if stats.can_make_call()
        ]
        ...
        def token_score(token: str) -> tuple:
            ...
            return (
                ...
                if stats.health == TokenHealth.DEGRADED
                else 2,
        ...
        # Pick the best score, then round-robin among tokens that tie for it so
        # equally-healthy tokens share load instead of always selecting the first.
        best_score = min(token_score(token) for token in available_tokens)
        tied = [token for token in available_tokens if token_score(token) == best_score]
        best_token = tied[self._round_robin_index % len(tied)]
    ...
    def update_token_rate_limit(
        self, token: str, calls_remaining: int, reset_time: datetime | None = None
    ) -> None:
        """
    ...
    def get_health_report(self) -> dict[str, any]:
        ...
        report["overall_error_rate"] = report["total_errors"] / max(
    ...
    def get_health(self) -> dict[str, any]:
        """Health summary with manager-level request totals surfaced at top level."""
        report = self.get_health_report()
        manager_stats = report["manager_stats"]
        report["total_requests"] = manager_stats["total_requests"]
        report["successful_requests"] = manager_stats["successful_requests"]
        report["failed_requests"] = manager_stats["failed_requests"]
        return report
    ...
    async def get_token(self) -> str | None:
        """Get the best available token (alias for get_best_token)."""
        return await self.get_best_token()

    async def record_success(self, token: str, response_time: float = 0.0) -> None:
        """Record a successful API call for a token."""
        if token in self.tokens:
            self.tokens[token].record_call(response_time, success=True)
            self._total_requests += 1
            self._successful_requests += 1

    async def record_failure(self, token: str, exception: Exception) -> None:
        """Record a failed API call for a token."""
        if token in self.tokens:
            self.tokens[token].record_call(0.0, success=False)
            self._total_requests += 1
            self._failed_requests += 1

    def get_token_stats(self, token: str) -> object | None:
    ...
    async def execute_batch_with_concurrency(
        self, operations: list[callable], max_concurrent: int = 10, max_retries: int = 3
    ) -> list[any]:
        """
        ...
        results = await asyncio.gather(
            *[bounded_operation(op) for op in operations], return_exceptions=True
        )
