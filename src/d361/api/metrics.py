...
class MetricsConfig(BaseModel):
    ...
    max_series_points: int = Field(
        default=10000, ge=100, le=100000, description="Maximum points per metric series"
    )

    retention_hours: int = Field(
        default=24, ge=1, le=168, description="Hours to retain metrics data"
    )
    ...
    export_format: str = Field(
        default="prometheus", description="Export format (prometheus, json, influxdb)"
    )

    export_endpoint: str | None = Field(
        default=None, description="Endpoint for metrics export"
    )

    # Alerting settings
    enable_alerts: bool = Field(
        default=False, description="Enable alerting on thresholds"
    )

    alert_thresholds: dict[str, float] = Field(
        default_factory=dict, description="Alert thresholds for metrics"
    )

    # Custom labels
    default_labels: dict[str, str] = Field(
        default_factory=dict, description="Default labels applied to all metrics"
    )
...
class ApiMetrics:
    ...
    def _init_builtin_metrics(self) -> None:
        ...
        self.create_metric(
            "api_request_duration_seconds", MetricType.HISTOGRAM, "API request duration"
        )
        self.create_metric(
            "api_request_size_bytes", MetricType.HISTOGRAM, "API request size"
        )
        self.create_metric(
            "api_response_size_bytes", MetricType.HISTOGRAM, "API response size"
        )
        ...
        self.create_metric(
            "api_rate_limit_remaining", MetricType.GAUGE, "Rate limit remaining"
        )
        self.create_metric(
            "api_rate_limit_reset_time", MetricType.GAUGE, "Rate limit reset time"
        )
        ...
        # Connection metrics
        self.create_metric(
            "api_connections_active", MetricType.GAUGE, "Active connections"
        )
        self.create_metric(
            "api_connections_created", MetricType.COUNTER, "Connections created"
        )
        self.create_metric(
            "api_connections_failed", MetricType.COUNTER, "Failed connections"
        )
        ...
        self.create_metric(
            "circuit_breaker_failures", MetricType.COUNTER, "Circuit breaker failures"
        )
        self.create_metric(
            "circuit_breaker_successes", MetricType.COUNTER, "Circuit breaker successes"
        )

        # Sync metrics
        self.create_metric(
            "sync_operations_total", MetricType.COUNTER, "Total sync operations"
        )
        self.create_metric(
            "sync_items_processed", MetricType.COUNTER, "Items processed in sync"
        )
        self.create_metric(
            "sync_duplicates_found", MetricType.COUNTER, "Duplicates found"
        )
        self.create_metric(
            "sync_duration_seconds", MetricType.HISTOGRAM, "Sync operation duration"
        )
    ...
    def record_request(
        ...
        # Coerce sizes to int at the boundary; callers may pass strings or other
        # non-numeric values, so fall back to 0 rather than raising on compare.
        ...
        request_size = _as_int(request_size)
        response_size = _as_int(response_size)
    ...
    def record_rate_limit(
        self, remaining: int, reset_time: float, hit: bool = False
    ) -> None:
    ...
    def record_token_rotation(
        self, old_token: str, new_token: str, reason: str = ""
    ) -> None:
    ...
    def record_cache_event(self, event: str, size: int | None = None) -> None:
    ...
    def record_circuit_breaker(
        self, name: str, event: str, state: str | None = None
    ) -> None:
    ...
    def record_sync_operation(
        self, operation: str, items: int = 0, duration: float = 0, duplicates: int = 0
    ) -> None:
    ...
    def get_metric(
        self, name: str, window: TimeWindow = TimeWindow.FIVE_MINUTES
    ) -> dict[str, Any] | None:
    ...
    def get_all_metrics(
        self, window: TimeWindow = TimeWindow.FIVE_MINUTES
    ...
    def get_summary(self) -> dict[str, Any]:
        """Get an overall summary of collected API request metrics.

        Aggregates the request counters and duration histogram across all
        recorded points (since start) into a flat summary dictionary.
        """
        with self._lock:

            def _total(name: str) -> float:
                series = self._series.get(name)
                return sum(p.value for p in series.points) if series else 0.0

            total_requests = int(_total("api_requests_total"))
            total_errors = int(_total("api_errors_total"))
            successful = total_requests - total_errors

            duration_series = self._series.get("api_request_duration_seconds")
            durations = (
                [p.value for p in duration_series.points] if duration_series else []
            )
    ...
    def get_health_metrics(self) -> dict[str, Any]:
        ...
        if requests_metric:
            metrics["request_rate_per_minute"] = (
                requests_metric["data"].get("rate", 0) * 60
            )

        # Average response time (last 5 minutes)
        duration_metric = self.get_metric(
            "api_request_duration_seconds", TimeWindow.FIVE_MINUTES
        )
        if duration_metric:
        ...
        # Active connections
        connections_metric = self.get_metric(
            "api_connections_active", TimeWindow.FIVE_MINUTES
        )
        if connections_metric:
            metrics["active_connections"] = connections_metric["data"].get("current", 0)

        # Rate limit status
        rate_limit_metric = self.get_metric(
            "api_rate_limit_remaining", TimeWindow.FIVE_MINUTES
        )
        if rate_limit_metric:
            metrics["rate_limit_remaining"] = rate_limit_metric["data"].get(
                "current", 0
            )
                ...
                if series.metric_type == MetricType.COUNTER:
                    ...
                    for key, value in aggregated.items():
                        if key.startswith("p"):
                            ...
                            # Standard metrics
                            suffix = (
                                f"_{key}" if key not in ["count", "sum"] else f"_{key}"
                            )
                            labels_str = self._format_prometheus_labels(series.labels)
    ...
    async def _export_to_endpoint(self) -> None:
        ...
        try:
            self.export_metrics(self.config.export_format)

            # Here you would send to your monitoring system
            # Example: POST to Prometheus pushgateway, InfluxDB, etc.
            logger.debug(
                f"Exported {len(self._series)} metrics to {self.config.export_endpoint}"
            )
...
_global_metrics: ApiMetrics | None = None
