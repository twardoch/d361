...
class OperationRequest:
    ...
    data: dict[str, Any] | None = None  # Data for create/update operations
    ...
    def __post_init__(self):
        ...
        if (
            self.operation_type in [OperationType.CREATE, OperationType.UPDATE]
            and not self.data
        ):
...
class BulkFetchResult:
    """Result of a bulk fetch, splitting successes from failures."""
...
class BulkOperationConfig(BaseModel):
    ...
    retry_failed_operations: bool = Field(
        default=True, description="Whether to retry failed operations"
    )

    max_retries_per_operation: int = Field(
        default=3, ge=0, le=10, description="Maximum retries per failed operation"
    )

    fail_fast: bool = Field(
        default=False, description="Stop on first error instead of continuing"
    )

    progress_callback: Callable[[int, int], None] | None = Field(
        default=None, description="Progress callback function (completed, total)"
    )
...
class BulkOperationManager:
    ...
    def __init__(
        self, client: Document360ApiClient, config: BulkOperationConfig | None = None
    ):
        """
    ...
    async def bulk_fetch_articles(self, article_ids: list[str]) -> BulkFetchResult:
        """Fetch multiple articles concurrently, collecting successes and failures.

        Each id is fetched via ``client.get_article`` under the concurrency
        semaphore; failures are captured per id rather than aborting the batch.
        """
        result = BulkFetchResult()

        async def _fetch(article_id: str) -> None:
            async with self._semaphore:
                try:
                    article = await self.client.get_article(article_id)
                    result.successful.append(article)
                except Exception as exc:
                    result.failed.append({"id": article_id, "error": str(exc)})

        await asyncio.gather(*(_fetch(article_id) for article_id in article_ids))
        return result

    async def execute_bulk_create(
        self, articles_data: list[dict[str, Any]], chunk_size: int | None = None
    ) -> BulkOperationResult:
        """
    ...
    async def execute_bulk_delete(
        self, article_ids: list[str], chunk_size: int | None = None
    ) -> BulkOperationResult:
        """
    ...
    async def _execute_bulk_operations(
        self, requests: list[OperationRequest], chunk_size: int | None = None
    ) -> BulkOperationResult:
        """
        ...
        for chunk_idx, chunk in enumerate(chunks):
            logger.debug(
                f"Processing chunk {chunk_idx + 1}/{len(chunks)} with {len(chunk)} operations"
            )
            ...
            for request, chunk_result in zip(chunk, chunk_results):
                ...
                if isinstance(chunk_result, Exception):
                    logger.error(
                        f"Unexpected error in operation {request.item_id}: {chunk_result}"
                    )
        ...
        if self.config.retry_failed_operations and result.failed_requests_details:
            logger.info(
                f"Retrying {len(result.failed_requests_details)} failed operations"
            )
            retry_result = await self._retry_failed_operations(
                result.failed_requests_details
            )
    ...
    async def _retry_failed_operations(
        self, failed_requests: list[OperationRequest]
    ) -> BulkOperationResult:
        """
        ...
        """
        result = BulkOperationResult(
            operation_type=failed_requests[0].operation_type
            if failed_requests
            else OperationType.FETCH,
        ...
        retry_candidates = [
            req for req in failed_requests if req.attempts < req.max_attempts
        ]
...
class SmartBulkProcessor:
    ...
    async def process_articles_intelligently(
        self,
        operation: str,
        articles_data: list[dict[str, Any] | tuple[str, dict[str, Any]] | str],
