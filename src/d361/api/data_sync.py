...
class ContentFingerprint:
    ...
    def from_article(cls, article: Article) -> ContentFingerprint:
        ...
        metadata_hash = hashlib.sha256(
            f"{title}:{article.category_id}".encode()
        ).hexdigest()[:16]
    ...
    def similarity_score(self, other: ContentFingerprint) -> float:
        ...
        elif self.content_length > 0 and other.content_length > 0:
            length_ratio = min(self.content_length, other.content_length) / max(
                self.content_length, other.content_length
            )
            word_ratio = (
                min(self.word_count, other.word_count)
                / max(self.word_count, other.word_count)
                if max(self.word_count, other.word_count) > 0
                else 0
            )
            avg_content_similarity = (length_ratio + word_ratio) / 2
            score += (
                0.3 * avg_content_similarity
            )  # 30% weight for partial content similarity
        ...
        elif self.paragraph_count > 0 and other.paragraph_count > 0:
            para_ratio = min(self.paragraph_count, other.paragraph_count) / max(
        ...
        if self.title and other.title:
            if self.title.lower() == other.title.lower():
                ...
                if title1_words and title2_words:
                    ...
                    )  # 10% weight for partial title similarity
...
class ChangeRecord:
    ...
    # Content tracking
    old_fingerprint: ContentFingerprint | None = None
    new_fingerprint: ContentFingerprint | None = None
...
class SyncConfig(BaseModel):
    ...
    change_detection_window: int = Field(
        default=7, ge=1, le=90, description="Days to look back for change detection"
    )
    ...
    auto_resolve_conflicts: bool = Field(
        default=False, description="Automatically resolve conflicts when possible"
    )

    prefer_newer_content: bool = Field(
        default=True, description="Prefer newer content in conflicts"
    )

    # State management
    save_sync_state: bool = Field(
        default=True, description="Save sync state for incremental operations"
    )

    state_retention_days: int = Field(
        default=30, ge=1, le=365, description="Days to retain sync state"
    )

    # Performance
    enable_caching: bool = Field(default=True, description="Enable fingerprint caching")
...
class SyncState(BaseModel):
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        return {
            ...
            "item_fingerprints": {
                item_id: fp.to_dict() for item_id, fp in self.item_fingerprints.items()
            },
...
class DataSyncManager:
    ...
    def __init__(
        ...
        # Internal state
        self._current_sync: SyncState | None = None
    ...
    async def deduplicate_articles(self, articles: list[Article]) -> list[Article]:
        """Return articles with duplicates removed, keeping first occurrences.

        Articles sharing an identical content hash are always treated as
        duplicates. For CONTENT/HYBRID strategies, near-duplicates whose
        fingerprint similarity meets ``similarity_threshold`` are also removed.
        """
        unique: list[Article] = []
        ...
        for article in articles:
            fp = ContentFingerprint.from_article(article)

            if fp.content_hash in seen_hashes:
                continue

            if self.config.deduplication_strategy in (
                DeduplicationStrategy.CONTENT,
                DeduplicationStrategy.HYBRID,
            ) and any(
                fp.similarity_score(existing) >= self.config.similarity_threshold
    ...
    async def detect_changes(
        ...
        """Compare two article snapshots and classify additions/updates/deletions.

        An article is 'updated' when its content fingerprint hash differs between
        snapshots for the same id.
        """
        old_by_id = {article.id: article for article in old_articles}
        new_by_id = {article.id: article for article in new_articles}

        added = [article for aid, article in new_by_id.items() if aid not in old_by_id]
        deleted = [
            article for aid, article in old_by_id.items() if aid not in new_by_id
        ]
        updated = [
            new_article
            for aid, new_article in new_by_id.items()
            if aid in old_by_id
            and ContentFingerprint.from_article(old_by_id[aid]).content_hash
            != ContentFingerprint.from_article(new_article).content_hash
        ]
    ...
    async def sync_articles(
        ...
        try:
            ...
            current_articles = await self._fetch_articles(
                category_id, project_version_id
            )
            ...
            if sync_state.strategy in [SyncStrategy.INCREMENTAL, SyncStrategy.DELTA]:
                ...
                logger.info(
                    f"Loaded {len(previous_fingerprints)} previous fingerprints"
                )

            # Process articles in batches
            await self._process_articles_batch(
                current_articles, sync_state, previous_fingerprints
            )
            ...
            logger.info(
                "Article synchronization completed",
                sync_id=sync_state.sync_id,
    ...
    async def _initialize_sync(
        self, sync_id: str | None, operation_type: str, operation_params: dict[str, Any]
    ) -> SyncState:
    ...
    async def _fetch_articles(
        self, category_id: str | None, project_version_id: str | None
    ) -> list[Article]:
        ...
        try:
            if category_id:
                ...
                for category in categories:
                    category_articles = await self.api_client.get_articles_by_category(
                        category.id
                    )
    ...
    async def _process_articles_batch(
        ...
        async def process_article(article: Article) -> None:
            async with semaphore:
                await self._process_single_article(
                    article, sync_state, previous_fingerprints
                )
        ...
        for i in range(0, len(articles), self.config.batch_size):
            ...
            logger.info(
                f"Processing batch {i // self.config.batch_size + 1}/{(len(articles) - 1) // self.config.batch_size + 1}"
            )
        ...
        try:
            ...
            if article.id in previous_fingerprints:
                ...
                if similarity < 1.0:  # Content changed
                    ...
                    change_record.field_changes = self._detect_field_changes(
            ...
            sync_state.average_processing_time = (
                sync_state.total_processing_time / sync_state.processed_items
            )
    ...
    def _detect_field_changes(
        self, old_fp: ContentFingerprint, new_fp: ContentFingerprint
    ) -> dict[str, Any]:
        ...
        if old_fp.title != new_fp.title:
            changes["title"] = {"old": old_fp.title, "new": new_fp.title}

        if old_fp.category != new_fp.category:
            changes["category"] = {"old": old_fp.category, "new": new_fp.category}
    ...
    async def _deduplicate_articles(self, sync_state: SyncState) -> None:
        ...
        logger.info(
            f"Starting deduplication with strategy: {self.config.deduplication_strategy.value}"
        )
    ...
    async def _load_previous_fingerprints(self) -> dict[str, ContentFingerprint]:
        ...
        try:
            ...
            logger.info(
                f"Loaded {len(fingerprints)} previous fingerprints from {latest_file.name}"
            )
            return fingerprints
    ...
    async def _load_sync_state(self, sync_id: str) -> SyncState | None:
    ...
    async def get_sync_report(self, sync_id: str) -> dict[str, Any] | None:
        ...
        return {
            ...
            "completed_at": sync_state.completed_at.isoformat()
            if sync_state.completed_at
            else None,
            "duration": (
                sync_state.completed_at - sync_state.started_at
            ).total_seconds()
            if sync_state.completed_at
            else None,
            "progress": {
            ...
            "changes": {
                ...
                "added": sum(
                    1
                    for c in sync_state.detected_changes
            ...
            "deduplication": {
                ...
                "total_duplicates": sum(
                    len(group) for group in sync_state.duplicate_groups.values()
                ),
            ...
            "performance": {
                ...
                "items_per_second": sync_state.processed_items
                / max(sync_state.total_processing_time, 0.001),
            },
        }
