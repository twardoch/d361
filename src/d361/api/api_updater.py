...
from .generate_models import ModelGenerationConfig, ModelGenerator
from .openapi_integration import OpenApiConfig, OpenApiIntegration, OpenApiSpec
...
class ApiUpdaterConfig(BaseModel):
    ...
    openapi_config: OpenApiConfig = Field(
        default_factory=OpenApiConfig, description="OpenAPI integration configuration"
    )
    ...
    auto_update_enabled: bool = Field(
        default=False, description="Enable automatic updates when changes are detected"
    )

    # Notification and logging
    log_updates: bool = Field(default=True, description="Log update events")
    ...
    on_update_start: str | None = Field(
        None, description="Callback function name for update start"
    )

    on_update_complete: str | None = Field(
        None, description="Callback function name for update completion"
    )

    on_update_failed: str | None = Field(
        None, description="Callback function name for update failure"
    )
...
class ApiUpdater:
    ...
    def __init__(
        self,
        config: ApiUpdaterConfig | None = None,
    ...
    async def _monitoring_loop(self) -> None:
        ...
        logger.info(
            f"Starting monitoring loop with {self.config.check_interval_seconds}s interval"
        )
    ...
    async def update_api_client(
        self, trigger: UpdateTrigger = UpdateTrigger.MANUAL, force: bool = False
    ) -> UpdateEvent:
        """
        ...
        event = UpdateEvent(
            timestamp=start_time, trigger=trigger, status=UpdateStatus.RUNNING
        )
        ...
        try:
            ...
            generation_result = await self.model_generator.generate_models(
                force_refresh=True
            )
            ...
            logger.info(
                "API client update completed successfully",
    ...
    async def _get_spec_changes(
        self, old_hash: str, new_spec: OpenApiSpec
    ) -> list[str]:
    ...
    def get_update_history(self, limit: int | None = None) -> list[UpdateEvent]:
        """
        Get update history.
...
async def create_updater(
    ...
    config = ApiUpdaterConfig(
        check_interval_seconds=check_interval_seconds, auto_update_enabled=auto_update
    )
...
async def run_one_time_update(
    output_dir: str | None = None, force: bool = False
) -> UpdateEvent:
    """
    ...
    try:
        return await updater.update_api_client(
            trigger=UpdateTrigger.MANUAL, force=force
        )
    finally:
        await updater.close()
