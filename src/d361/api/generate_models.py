...
from .openapi_integration import OpenApiConfig, OpenApiIntegration
...
class ModelGenerationConfig(BaseModel):
    ...
    class_name_prefix: str = Field(
        default="", description="Prefix for generated class names"
    )

    class_name_suffix: str = Field(
        default="", description="Suffix for generated class names"
    )

    use_enum_field: bool = Field(
        default=True, description="Use Enum for string fields with constraints"
    )

    use_union_operator: bool = Field(
        default=True, description="Use union operator (|) for Union types"
    )

    target_python_version: str = Field(
        default="3.12", description="Target Python version"
    )

    use_double_quotes: bool = Field(
        default=True, description="Use double quotes for strings"
    )

    use_schema_description: bool = Field(
        default=True, description="Include schema descriptions as docstrings"
    )

    use_field_description: bool = Field(
        default=True, description="Include field descriptions"
    )

    snake_case_field: bool = Field(
        default=True, description="Convert camelCase to snake_case for field names"
    )

    strict_nullable: bool = Field(default=True, description="Use strict nullable types")

    # Additional options
    enable_faux_immutability: bool = Field(
        default=False, description="Enable faux immutability (frozen models)"
    )

    use_annotated: bool = Field(default=True, description="Use Annotated types")
...
class ModelGenerator:
    ...
    def __init__(self, config: ModelGenerationConfig | None = None):
        """
    ...
    async def _generate_with_datamodel_codegen(
        self, spec: Any, output_file: Path
    ) -> GenerationResult:
        """Generate models using datamodel-code-generator."""
        ...
        try:
            ...
            if self.config.class_name_prefix:
                cmd.extend(
                    [
                        "--class-name",
                        f"{self.config.class_name_prefix}{{name}}{self.config.class_name_suffix}",
                    ]
                )
