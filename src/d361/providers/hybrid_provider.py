...
class HybridConfig(BaseModel):
    ...
    max_age_hours: int = Field(
        default=24, ge=1, description="Maximum age for cached data"
    )
    realtime_threshold_minutes: int = Field(
        default=5, ge=1, description="Threshold for realtime data"
    )
    fresh_threshold_hours: int = Field(
        default=1, ge=1, description="Threshold for fresh data"
    )
    ...
    parallel_requests: bool = Field(
        default=True, description="Enable parallel source requests"
    )
    max_concurrent_sources: int = Field(
        default=2, ge=1, le=5, description="Max concurrent source requests"
    )
    source_timeout: float = Field(
        default=30.0, ge=1.0, description="Timeout per source in seconds"
    )

    # Fallback behavior
    enable_fallback: bool = Field(
        default=True, description="Enable fallback to other sources"
    )
    min_sources_required: int = Field(
        default=1, ge=1, description="Minimum sources that must succeed"
    )

    # Conflict resolution
    conflict_resolution_strategy: str = Field(
        default="newest_first", description="Strategy for resolving data conflicts"
    )

    # Quality thresholds
    min_content_quality: float = Field(
        default=0.7, ge=0.0, le=1.0, description="Minimum quality threshold for content"
    )
...
class HybridProvider(DataProvider):
    ...
    def __init__(
        ...
        api_provider: Any | None = None,
        archive_provider: Any | None = None,
        scraper_provider: Any | None = None,
        ...
        **kwargs: Any,
        ...
        Args:
            ...
            source_priority: Ordered list of source names to try (e.g. ``["api", "archive"]``)
            freshness_threshold_hours: Maximum age (hours) for data to be considered fresh
            conflict_resolution: Strategy when sources disagree (``"latest"`` uses newest ``updated_at``)
            enable_caching: Cache responses locally
            collect_metrics: Collect per-source performance metrics
        ...
        self.source_priority: list[str] = source_priority or [
            "api",
            "archive",
            "scraper",
        ]
        ...
        self._performance_calls: list[dict[str, Any]] = []
    ...
    def _source_map(self) -> dict[str, Any]:
        """Return mapping from source name string to provider instance."""
        return {
            "api": self.api_provider,
            "archive": self.archive_provider,
            "scraper": self.scraper_provider,
            "cache": self.cache_provider,
        }

    async def _fetch_article_from_provider(
        self, provider: Any, article_id: Any, **kwargs: Any
    ) -> Article:
        """Delegate get_article call to a concrete provider."""
        return await provider.get_article(article_id, **kwargs)

    def get_metrics(self) -> dict[str, Any]:
        """Alias for :meth:`get_performance_metrics`."""
    ...
    def get_performance_metrics(self) -> dict[str, Any]:
        ...
        Returns:
            dict: Metrics keyed by source name.
        """
        result: dict[str, Any] = {}
        ...
        result["calls"] = list(self._performance_calls)
        result["total_requests"] = len(self._performance_calls)
        result["request_count"] = len(self._performance_calls)
        return result

    async def get_article(self, article_id: Any, **kwargs: Any) -> Article:
        """Fetch article using the best available source.

        When ``check_all_sources=True`` all configured sources are queried
        and the result is chosen according to ``conflict_resolution``.  When
        ``require_fresh=True`` only sources that returned data newer than
        ``freshness_threshold_hours`` are considered.
        ...
        Args:
            ...
            **kwargs: Additional parameters.  Supported keys:
                ...
                ``require_fresh`` (bool)
                    Only return data fresher than ``freshness_threshold_hours``.

                ``check_all_sources`` (bool)
                    Query all available sources and apply conflict resolution.
        ...
        """
        kwargs.pop("require_fresh", False)
        check_all_sources: bool = kwargs.pop("check_all_sources", False)

        source_map = self._source_map()
        providers_ordered = [
            (name, source_map[name])
            for name in self.source_priority
            if source_map.get(name) is not None
        ]

        if check_all_sources and self.conflict_resolution == "latest":
            # Query all sources and pick the one with the newest updated_at
            results: list[Article] = []
            for _name, provider in providers_ordered:
                try:
                    art = await self._fetch_article_from_provider(
                        provider, article_id, **kwargs
                    )
                    results.append(art)
                except Exception:
                    continue
            ...
            if not results:
                raise Document360Error(
                    f"Article '{article_id}' not found in any source",
                    category=ErrorCategory.NOT_FOUND,
                    severity=ErrorSeverity.MEDIUM,
                )

            def _updated_key(a: Article) -> datetime:
                if a.updated_at is None:
                    return datetime.min.replace(tzinfo=timezone.utc)
                if a.updated_at.tzinfo is None:
                    return a.updated_at.replace(tzinfo=timezone.utc)
                return a.updated_at

            return max(results, key=_updated_key)

        # Standard: try sources in priority order with fallback
        ...
        for _name, provider in providers_ordered:
            try:
                ...
                if self.collect_metrics:
                    self._performance_calls.append(
                        {"source": _name, "article_id": str(article_id)}
                    )
                return art
    ...
    async def _get_article_legacy(self, article_id: int, **kwargs: Any) -> Article:
        """Legacy fetch path kept for internal strategy methods.
        ...
        try:
            result = await self._fetch_with_strategy(
                request, self._fetch_article_from_source
            )
            return result
    ...
    async def get_category(self, category_id: int, **kwargs: Any) -> Category:
        ...
        try:
            result = await self._fetch_with_strategy(
                request, self._fetch_category_from_source
            )
            return result
        except Exception as e:
    ...
    async def list_categories(self, **kwargs: Any) -> list[Category]:
        ...
        try:
            result = await self._fetch_with_strategy(
                request, self._fetch_categories_list_from_source
            )
            return result
        except Exception as e:
    ...
    async def get_project_version(self, **kwargs: Any) -> ProjectVersion:
        ...
        try:
            result = await self._fetch_with_strategy(
                request, self._fetch_project_version_from_source
            )
            return result
        except Exception as e:
    ...
    async def _fetch_with_strategy(self, request: DataRequest, fetch_func: Any) -> Any:
        ...
        sources = self._determine_source_priority(
            request.freshness, request.preferred_sources, request.fallback_sources
        )
    ...
    async def _fetch_parallel(
        self, request: DataRequest, fetch_func: Any, sources: list[DataSource]
    ) -> Any:
    ...
    async def _fetch_sequential(
        ...
        for source in sources:
            try:
                result = await self._fetch_from_source_with_metrics(
                    source, request, fetch_func
                )
                if result is not None:
                    self._cache_data(request, result)
                    return result
    ...
    async def _fetch_from_source_with_metrics(
        self, source: DataSource, request: DataRequest, fetch_func: Any
    ) -> Any:
        ...
        try:
            result = await asyncio.wait_for(
                fetch_func(source, request), timeout=self.config.source_timeout
            )
            ...
            metrics.error_rate = metrics.error_count / (
    ...
    def _source_meets_freshness(
        self, source: DataSource, freshness: FreshnessRequirement
    ) -> bool:
    ...
    def _get_cached_data(self, request: DataRequest) -> Any | None:
    ...
    async def _fetch_article_from_source(
    ...
    async def _fetch_articles_list_from_source(
        ...
        **kwargs: Any,
        ...
        if source == DataSource.API and self.api_provider:
            return await self.api_provider.list_articles(
                category_id=category_id, status=status, **kwargs
            )
        elif source == DataSource.ARCHIVE and self.archive_provider:
            return await self.archive_provider.list_articles(
                category_id=category_id, status=status, **kwargs
            )
        elif source == DataSource.SCRAPER and self.scraper_provider:
            raise NotImplementedError("Scraper provider not yet implemented")
    ...
    async def _fetch_category_from_source(
        self, source: DataSource, request: DataRequest
    ) -> Category:
    ...
    async def _fetch_categories_list_from_source(
        self, source: DataSource, request: DataRequest
    ) -> list[Category]:
    ...
    async def _fetch_project_version_from_source(
        self, source: DataSource, request: DataRequest
    ) -> ProjectVersion:
    ...
    async def _stream_from_source(
        self, source: DataSource, **kwargs: Any
