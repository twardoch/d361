...
class ExtractionConfig(BaseModel):
    ...
    min_content_length: int = Field(
        default=50, ge=10, description="Minimum content length to consider valid"
    )

    max_title_length: int = Field(
        default=200, ge=50, description="Maximum title length"
    )

    normalize_whitespace: bool = Field(
        default=True, description="Normalize whitespace in extracted text"
    )

    extract_links: bool = Field(
        default=True, description="Extract and process internal/external links"
    )

    extract_images: bool = Field(
        default=True, description="Extract image metadata and URLs"
    )

    # Language detection
    detect_language: bool = Field(
        default=True, description="Attempt to detect content language"
    )
...
class ExtractedContent:
    ...
    def is_high_quality(self) -> bool:
        ...
        return (
            self.quality in [ExtractionQuality.EXCELLENT, ExtractionQuality.GOOD]
...
class ContentExtractor:
    ...
    def extract(self, html: str, url: str) -> ExtractedContent:
        ...
        try:
            ...
            content_text = (
                self._extract_text(article_content) if article_content else ""
            )
            ...
            images = (
                self._extract_images(soup, url) if self.config.extract_images else []
            )
    ...
    def _extract_article_content(self, soup: BeautifulSoup) -> Tag | None:
    ...
    def _detect_content_type(
        self, title: str, content: str, soup: BeautifulSoup
    ) -> ContentType:
        ...
        if any(
            ...
            for keyword in ["faq", "frequently asked", "questions"]
        ):
        ...
        if any(
            keyword in title_lower
            for keyword in ["tutorial", "how to", "step by step", "guide"]
        ):
        ...
        if any(
            keyword in title_lower
            for keyword in ["guide", "getting started", "quickstart"]
        ):
        ...
        if any(
            keyword in title_lower
            for keyword in ["reference", "api", "documentation", "spec"]
        ):
