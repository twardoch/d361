...
class ConversionConfig(BaseModel):
    ...
    style: MarkdownStyle = Field(
        default=MarkdownStyle.DOCUMENT360, description="Markdown style to use"
    )
    ...
    base_url: str | None = Field(
        None, description="Base URL for relative link resolution"
    )
    ...
    bullet_style: str = Field(
        default="-", description="Bullet list style: '-', '*', or '+'"
    )

    emphasis_style: str = Field(default="*", description="Emphasis style: '*' or '_'")

    strong_style: str = Field(
        default="**", description="Strong emphasis style: '**' or '__'"
    )

    # Code formatting
    code_language_detection: bool = Field(
        default=True, description="Attempt to detect code block languages"
    )

    fence_code_blocks: bool = Field(
        default=True, description="Use fenced code blocks (```)"
    )

    # Table handling
    convert_tables: bool = Field(
        default=True, description="Convert HTML tables to Markdown tables"
    )

    # Image handling
    download_images: bool = Field(
        default=False, description="Download and localize images"
    )

    image_directory: Path | None = Field(
        None, description="Directory for downloaded images"
    )

    # Quality control
    validate_markdown: bool = Field(
        default=True, description="Validate generated markdown"
    )

    max_line_length: int | None = Field(
        default=None, ge=80, description="Maximum line length for wrapping"
    )
...
class ConversionResult:
    ...
    def is_high_quality(self) -> bool:
        ...
        return (
            self.conversion_quality >= 0.8
            and len(self.warnings) < 5
            and len(self.markdown.strip()) > 0
        )
...
class MarkdownConverter:
    ...
    def convert(self, html: str, title: str | None = None) -> ConversionResult:
        """
    ...
    def _preprocess_html(
        self, soup: BeautifulSoup, stats: ConversionStats
    ) -> BeautifulSoup:
        ...
        for i, line in enumerate(lines):
            if line.strip().startswith(("- ", "* ", "+ ")) or re.match(
                ...
                if (
                    ...
                    and not re.match(r"^\d+\. ", fixed_lines[-1].strip())
                ):
                    fixed_lines.append("")
    ...
    def _assess_conversion_quality(
        self, html: str, markdown: str, stats: ConversionStats
    ) -> float:
