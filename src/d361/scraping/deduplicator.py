...
class DeduplicationConfig(BaseModel):
    ...
    exact_threshold: float = Field(
        default=1.0, ge=0.95, le=1.0, description="Threshold for exact duplicates"
    )

    near_duplicate_threshold: float = Field(
        default=0.9, ge=0.8, le=0.99, description="Threshold for near duplicates"
    )

    similar_threshold: float = Field(
        default=0.7, ge=0.5, le=0.89, description="Threshold for similar content"
    )

    related_threshold: float = Field(
        default=0.5, ge=0.3, le=0.69, description="Threshold for related content"
    )
    ...
    normalize_content: bool = Field(
        default=True, description="Normalize content before comparison"
    )

    ignore_case: bool = Field(default=True, description="Ignore case when comparing")

    ignore_whitespace: bool = Field(
        default=True, description="Ignore whitespace differences"
    )

    ignore_punctuation: bool = Field(
        default=False, description="Ignore punctuation differences"
    )

    # Performance settings
    min_content_length: int = Field(
        default=100, ge=10, description="Minimum content length to process"
    )

    max_comparisons: int = Field(
        default=10000, ge=100, description="Maximum pairwise comparisons"
    )

    batch_size: int = Field(
        default=1000, ge=10, description="Batch size for processing"
    )

    # Shingle settings (for n-gram similarity)
    shingle_size: int = Field(
        default=3, ge=1, le=10, description="Size of n-gram shingles"
    )
...
class ContentDeduplicator:
    ...
    def find_duplicates(
        self, content_items: dict[str, str], return_groups: bool = True
    ) -> list[DuplicateGroup] | dict[str, DuplicateStatus]:
        """
        ...
        try:
            ...
            if len(filtered_items) != len(content_items):
                logger.info(
                    f"Filtered to {len(filtered_items)} items after length check"
                )
    ...
    def calculate_similarity(
        self, content1: str, content2: str, algorithm: SimilarityAlgorithm | None = None
    ) -> SimilarityScore:
        """
    ...
    def _find_similar_pairs(
        self, content_items: dict[str, str]
    ...
    def _verify_with_secondary_algorithms(
        self, similar_pairs: list[tuple[str, str, float]], content_items: dict[str, str]
    ) -> list[tuple[str, str, float]]:
        ...
        for item_id1, item_id2, primary_score in similar_pairs:
            ...
            if secondary_scores:
                average_score = (primary_score + sum(secondary_scores)) / (
                    len(secondary_scores) + 1
                )
            else:
    ...
    def _group_duplicates(
        self, similar_pairs: list[tuple[str, str, float]]
    ) -> list[DuplicateGroup]:
    ...
    def _create_status_dict(
        self, duplicate_groups: list[DuplicateGroup], content_items: dict[str, str]
