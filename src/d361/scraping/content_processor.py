...
class ContentProcessor:
    ...
    def _remove_d360_artifacts(self, content: str) -> str:
        ...
        content = re.sub(
            r"<a[^>]*data-d360[^>]*>(.*?)</a>", r"\1", content, flags=re.DOTALL
        )

        # Remove Document360 widgets/embeds
        content = re.sub(
            r'<div[^>]*class="[^"]*d360[^"]*"[^>]*>.*?</div>',
            "",
            content,
            flags=re.DOTALL,
        )
...
class Document360ContentProcessor(ContentProcessor):
    ...
    async def load_and_merge_content(
        self,
        archive_provider: DataProvider | None = None,
        api_provider: DataProvider | None = None,
        ...
        if data_source == "archive" and archive_provider:
            ...
            logger.info(
                "Loading content from both archive and API with intelligent merging"
            )
            return await self._merge_content_intelligently(
                archive_provider, api_provider
            )
    ...
    async def _merge_content_intelligently(
        self, archive_provider: DataProvider, api_provider: DataProvider
    ) -> tuple[list[Article], list[Category]]:
        ...
        archive_articles, api_articles = await asyncio.gather(
            archive_provider.list_articles(), api_provider.list_articles()
        )

        archive_categories, api_categories = await asyncio.gather(
            archive_provider.list_categories(), api_provider.list_categories()
        )
    ...
    def _merge_articles_with_conflict_resolution(
        self, archive_articles: list[Article], api_articles: list[Article]
    ) -> list[Article]:
        ...
        for article_id, archive_article in archive_by_id.items():
            if article_id in api_by_id:
                ...
                merged_article = self._merge_single_article(
                    archive_article, api_article
                )
    ...
    def _merge_categories_with_conflict_resolution(
        self, archive_categories: list[Category], api_categories: list[Category]
    ) -> list[Category]:
        ...
        for category_id, archive_category in archive_by_id.items():
            if category_id in api_by_id:
                ...
                merged_category = self._merge_single_category(
                    archive_category, api_category
                )
    ...
    def _merge_single_article(
        self, archive_article: Article, api_article: Article
    ) -> Article:
        ...
        if (
            api_article.updated_at
            and archive_article.updated_at
            and api_article.updated_at > archive_article.updated_at
        ):
            base_article = api_article
            other_article = archive_article
            logger.debug(
                f"Using API version as base for article {base_article.id} (newer)"
            )
        else:
        ...
        if not merged.content and other_article.content:
            ...
            logger.debug(
                f"Using content from other source for article {merged.id} (base had no content)"
            )
            ...
            if len(other_article.content) > len(merged.content) * 1.5:
                logger.debug(
                    f"Using content from other source for article {merged.id} (richer content)"
                )
                merged.content = other_article.content

        return merged

    def _merge_single_category(
        self, archive_category: Category, api_category: Category
    ) -> Category:
        ...
        if (
            api_category.updated_at
            and archive_category.updated_at
            and api_category.updated_at > archive_category.updated_at
        ):
            return api_category
        else:
    ...
    async def enhance_content_with_advanced_processing(
        self, articles: list[Article], use_parallel: bool = True
    ) -> list[Article]:
        ...
        logger.info(
            f"Content enhancement completed: {len(enhanced_articles)} articles processed"
        )
        self.stats["articles_processed"] += len(enhanced_articles)

        return enhanced_articles

    async def _enhance_content_sequential(
        ...
        for article in articles:
            try:
                ...
                if len(enhanced_articles) % 50 == 0:
                    logger.info(
                        f"Enhanced {len(enhanced_articles)}/{len(articles)} articles"
                    )
    ...
    def _enhance_single_article_sync(self, article: Article) -> Article:
        ...
        try:
            return loop.run_until_complete(
                self._enhance_single_article_content(article)
            )
        finally:
            loop.close()
    ...
    async def _enhance_single_article_content(self, article: Article) -> Article:
        ...
        Returns:
            Enhanced article
        """
        if not article.content or not self._content_needs_advanced_processing(
            article.content
        ):
            return article

        try:
            from .converter import ConversionConfig, Document360MarkdownConverter
            ...
            logger.debug(
                f"d361 MarkdownConverter not available for article {article.id}"
            )
            return article
        except Exception as e:
    ...
    def _content_needs_advanced_processing(self, content: str) -> bool:
        ...
        html_count = sum(
            1 for indicator in html_indicators if indicator in content_lower
        )
    ...
    async def resolve_duplicates_with_advanced_detection(
        ...
        try:
            ...
            logger.info(
                f"Using d361 ContentDeduplicator for advanced duplicate detection on {len(articles)} articles"
            )
            ...
            exact_dupes = sum(
                1 for group in duplicate_groups if group.status.value == "exact"
            )
            near_dupes = sum(
                1 for group in duplicate_groups if group.status.value == "near"
            )
            similar = sum(
                1 for group in duplicate_groups if group.status.value == "similar"
            )
            ...
            logger.debug(
                "d361 ContentDeduplicator not available, returning articles unchanged"
            )
            return articles
        except Exception as e:
            logger.warning(
                f"d361 ContentDeduplicator failed: {e}, returning articles unchanged"
            )
            return articles
    ...
    def _process_duplicate_detection_results(
            ...
            if group.status.value in ["exact", "near"]:
                ...
                for dup_id in duplicate_ids:
                    if dup_id in article_map:
                        ...
                        logger.debug(
                            f"Removing {group.status.value} duplicate article {dup_id}"
                        )
                ...
                for dup_id in duplicate_ids:
                    if dup_id in article_map:
                        articles_to_remove.add(dup_id)
                        logger.debug(
                            f"Merging similar article {dup_id} into {representative_id}"
                        )
        ...
        logger.info(
            f"Resolved {len(articles_to_remove)} duplicate articles using advanced detection"
        )
        return resolved_articles
    ...
    def _merge_similar_article_metadata(
        ...
        for similar in similar_articles:
            if (
                similar.content
                and len(similar.content) > len(enhanced.content or "") * 1.2
                and similarity_score > 0.8
            ):
                logger.debug(
                    f"Using richer content from similar article for {enhanced.id}"
                )
