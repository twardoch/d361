...
class ScrapingConfig(BaseModel):
    ...
    viewport_width: int = Field(
        default=1920, ge=800, le=3840, description="Browser viewport width"
    )
    ...
    delay_min_seconds: float = Field(
        default=1.0, ge=0.1, description="Minimum delay between requests"
    )

    delay_max_seconds: float = Field(
        default=3.0, ge=0.5, description="Maximum delay between requests"
    )

    concurrent_pages: int = Field(
        default=1, ge=1, le=10, description="Number of concurrent pages"
    )
    ...
    randomize_viewport: bool = Field(
        default=True, description="Randomize viewport size"
    )
    ...
    allowed_domains: set[str] = Field(
        default_factory=set, description="Allowed domains for crawling"
    )
    ...
    max_pages: int = Field(default=1000, ge=1, description="Maximum pages to scrape")

    # Respect settings
    respect_robots_txt: bool = Field(default=True, description="Respect robots.txt")

    max_requests_per_domain: int = Field(
        default=100, ge=1, description="Max requests per domain"
    )

    # Storage
    save_screenshots: bool = Field(default=False, description="Save page screenshots")

    screenshot_dir: Path | None = Field(None, description="Directory for screenshots")
...
class ScrapingSession(BaseModel):
    ...
    scraped_urls: set[str] = Field(
        default_factory=set, description="Successfully scraped URLs"
    )
    failed_urls: set[str] = Field(default_factory=set, description="Failed URLs")
    pending_urls: set[str] = Field(
        default_factory=set, description="URLs pending scraping"
    )
...
class Document360Scraper:
    ...
    def __init__(self, config: ScrapingConfig | None = None):
        ...
        logger.info(
            "Document360Scraper initialized",
    ...
    async def scrape_url(self, url: str) -> ScrapedPage:
        ...
        try:
            ...
            # Extract content
            scraped_page = await self._extract_page_content(
                page, url, load_time_ms, response.status
            )
            ...
            for selector in self.COOKIE_SELECTORS:
                try:
                    ...
                    if element:
                        await element.click()
                        logger.debug(
                            f"Dismissed cookie banner with selector: {selector}"
                        )
    ...
    async def _extract_page_content(
        self, page: Page, url: str, load_time_ms: float, status_code: int
    ) -> ScrapedPage:
    ...
    async def get_session_stats(self) -> ScrapingSession | None:
