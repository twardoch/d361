...
class ConfigValidator:
    ...
    def _validate_production_debug(self, config: AppConfig) -> list[str]:
        ...
        if self.environment == Environment.PRODUCTION:
            ...
            if config.monitoring.log_level.value in ["TRACE", "DEBUG"]:
                issues.append(
                    f"Log level '{config.monitoring.log_level}' too verbose for production"
                )
    ...
    def _validate_production_secrets(self, config: AppConfig) -> list[str]:
        ...
        if self.environment == Environment.PRODUCTION:
            ...
            if config.security.secrets_provider.value == "environment":
                issues.append(
                    "Consider using a dedicated secret manager (Vault, AWS Secrets Manager) in production instead of environment variables"
                )
    ...
    def _validate_production_monitoring(self, config: AppConfig) -> list[str]:
        ...
        if self.environment == Environment.PRODUCTION:
            ...
            if (
                config.monitoring.alerting_enabled
                and not config.monitoring.alert_webhook_url
            ):
    ...
    def _validate_resource_limits(self, config: AppConfig) -> list[str]:
        ...
        total_cache_mb = (
            config.cache.max_disk_size_mb + config.archive.max_extracted_size_mb
        )
        if total_cache_mb > 50000:  # 50GB
            issues.append(
                f"Total cache size ({total_cache_mb}MB) may be excessive and cause disk space issues"
            )

        # Check memory limits
        if config.cache.max_memory_mb > 4096:  # 4GB
            issues.append(
                f"Memory cache size ({config.cache.max_memory_mb}MB) may be too large"
            )

        # Check concurrent operations
        if config.scraping.concurrent_pages > 20:
            issues.append(
                f"Concurrent pages limit ({config.scraping.concurrent_pages}) may overwhelm target servers"
            )

        if config.api.bulk_concurrency > 50:
            issues.append(
                f"API bulk concurrency ({config.api.bulk_concurrency}) may exceed rate limits"
            )
    ...
    def _validate_network_settings(self, config: AppConfig) -> list[str]:
        ...
        if config.api.timeout_seconds < 5:
            issues.append(
                f"API timeout ({config.api.timeout_seconds}s) may be too short for reliable operations"
            )

        if config.scraping.timeout_seconds > 300:  # 5 minutes
            issues.append(
                f"Scraping timeout ({config.scraping.timeout_seconds}s) may be excessive"
            )
        ...
        if config.monitoring.metrics_enabled:
            if config.monitoring.metrics_port in ports_used:
                issues.append(
                    f"Port conflict: metrics port {config.monitoring.metrics_port}"
                )
        ...
        if config.monitoring.health_check_enabled:
            if config.monitoring.health_check_port in ports_used:
                issues.append(
                    f"Port conflict: health check port {config.monitoring.health_check_port}"
                )
    ...
    def _validate_database_settings(self, config: AppConfig) -> list[str]:
        ...
        if config.archive.vacuum_interval_hours > 168:  # 1 week
            issues.append(
                f"Database vacuum interval ({config.archive.vacuum_interval_hours}h) may be too long"
            )
...
class ConfigLoader:
    ...
    def __init__(
        ...
        self.validator: ConfigValidator | None = None
    ...
    async def load_configuration(
        ...
        try:
            ...
            if validate:
                validation_issues = await self._validate_configuration(
                    config, fail_on_validation_errors
                )
    ...
    async def _inject_secrets(self, config: AppConfig) -> AppConfig:
        ...
        try:
            ...
            if encryption_secret.is_valid():
                config_dict["security"]["encryption_key_path"] = Path(
    ...
    async def _validate_configuration(
        self, config: AppConfig, fail_on_errors: bool
    ) -> list[str]:
        """Validate configuration."""
    ...
    async def _handle_config_reload(self, new_config: AppConfig) -> None:
        ...
        try:
            ...
            if validation_issues:
                logger.warning(
                    f"New configuration has validation issues, keeping current config: {'; '.join(validation_issues)}"
                )
                return
    ...
    async def get_current_config(self) -> AppConfig | None:
        """Get the current configuration."""
        return self._current_config

    async def reload_configuration(
        self, validate: bool = True, fail_on_validation_errors: bool = None
    ) -> AppConfig:
        """
        ...
        return await self.load_configuration(
            environment, validate, fail_on_validation_errors
        )
    ...
    async def get_config_summary(self) -> dict[str, Any]:
        ...
        return {
            **base_summary,
            ...
            "loading_started_at": self._loading_started_at.isoformat()
...
def get_config_loader(
    base_config_dir: Path | None = None, environment_override: str | None = None
) -> ConfigLoader:
    """
