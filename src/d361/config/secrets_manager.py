...
class SecretsManager:
        ...
        for provider_type in fallback_chain:
            ...
            try:
                ...
                if health:
                    ...
                    logger.debug(
                        f"Fallback provider {provider_type} initialized successfully"
                    )
    ...
    async def get_secret(self, secret_id: str) -> SecretValue:
        ...
        for provider in self._fallback_providers:
            try:
                ...
                logger.info(
                    f"Secret '{secret_id}' retrieved from fallback provider: {provider.__class__.__name__}"
                )
                return secret
            except Exception as e:
                errors.append(f"{provider.__class__.__name__}: {e}")
                logger.debug(
                    f"Fallback provider {provider.__class__.__name__} failed for secret '{secret_id}': {e}"
                )
    ...
    async def set_secret(
        ...
        try:
            result = await self._primary_provider.set_secret(
                secret_id, value, secret_type, metadata
            )
    ...
    async def health_check(self) -> dict[str, bool]:
        ...
        if self._primary_provider:
            try:
                ...
                health_status[
                    f"primary_{self._primary_provider.__class__.__name__}"
                ] = False
    ...
    async def get_provider_info(self) -> dict[str, Any]:
        ...
        info = {
            ...
            "fallback_providers": [
                p.__class__.__name__ for p in self._fallback_providers
            ],
            "provider_health": await self.health_check(),
            "total_providers": 1 + len(self._fallback_providers)
            if self._primary_provider
            else len(self._fallback_providers),
...
def get_secrets_manager(config: SecurityConfig | None = None) -> SecretsManager:
    """
