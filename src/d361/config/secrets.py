...
class SecretsProvider(ABC):
    ...
    async def list_secrets(
        self, secret_type: SecretType | None = None, tags: dict[str, str] | None = None
    ) -> list[SecretMetadata]:
        """
...
class EnvironmentSecretsProvider(SecretsProvider):
    ...
    async def initialize(self) -> None:
        ...
        logger.debug(
            f"EnvironmentSecretsProvider initialized with prefix: {self.prefix}"
        )
    ...
    async def delete_secret(self, secret_id: str) -> bool:
        ...
        if env_var in os.environ:
            ...
            logger.debug(
                f"Secret '{secret_id}' deleted from environment variable '{env_var}'"
            )
            return True
        return False
...
class LocalFileSecretsProvider(SecretsProvider):
    ...
    def __init__(self, config: dict[str, Any]):
        ...
        self.secrets_dir = Path(
            config.get("secrets_dir", Path.home() / ".d361" / "secrets")
        )
    ...
    async def initialize(self) -> None:
        ...
        self._is_initialized = True
        logger.debug(
            f"LocalFileSecretsProvider initialized, secrets_dir: {self.secrets_dir}"
        )
        ...
        for secret_file in self.secrets_dir.glob("*.json"):
            try:
                ...
                metadata = SecretMetadata(
                    ...
                    secret_type=SecretType(
                        data.get("secret_type", SecretType.API_TOKEN)
                    ),
                    created_at=datetime.fromisoformat(data["created_at"]),
                    updated_at=datetime.fromisoformat(data["updated_at"]),
                    expires_at=datetime.fromisoformat(data["expires_at"])
                ...
                logger.warning(
                    f"Failed to read secret metadata from {secret_file}: {e}"
                )
                continue
    ...
    async def health_check(self) -> bool:
        ...
        return (
            ...
            and self.secrets_dir.exists()
            and self.secrets_dir.is_dir()
        )
...
class HashiCorpVaultProvider(SecretsProvider):
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            self._is_initialized = True
            logger.debug(
                f"HashiCorpVaultProvider initialized, vault_url: {self.vault_url}"
            )
    ...
    async def get_secret(self, secret_id: str) -> SecretValue:
        ...
        try:
            response = self._vault_client.secrets.kv.v2.read_secret(
                path=vault_path, mount_point=self.mount_point
            )
            ...
            metadata = SecretMetadata(
                secret_id=secret_id,
                secret_type=SecretType(
                    secret_data.get("secret_type", SecretType.API_TOKEN)
                ),
    ...
    async def set_secret(
        ...
        try:
            self._vault_client.secrets.kv.v2.create_or_update_secret(
                path=vault_path, secret=secret_data, mount_point=self.mount_point
            )
    ...
    async def delete_secret(self, secret_id: str) -> bool:
        ...
        try:
            ...
            logger.debug(
                f"Secret '{secret_id}' deleted from Vault at path '{vault_path}'"
            )
            return True
    ...
    async def list_secrets(
        ...
        try:
            response = self._vault_client.secrets.kv.v2.list_secrets(
                path=self.vault_path, mount_point=self.mount_point
            )
            ...
            for secret_id in response["data"]["keys"]:
                try:
                    ...
                    if tags and not all(
                        secret_value.metadata.tags.get(k) == v for k, v in tags.items()
                    ):
                        continue
                    ...
                    logger.warning(
                        f"Failed to get metadata for secret '{secret_id}': {e}"
                    )
                    continue
    ...
    async def health_check(self) -> bool:
        ...
        try:
            return (
                self._vault_client.sys.is_initialized()
                and self._vault_client.is_authenticated()
            )
        except Exception:
            return False
...
def create_secrets_provider(
    provider_type: SecretProvider, config: dict[str, Any]
) -> SecretsProvider:
    """
