...
class PluginManager:
    ...
    def __init__(self) -> None:
        ...
        self._provider_plugins: dict[
            str, type[Any]
        ] = {}  # Will be DataProvider when implemented
        ...
        try:
            ...
            try:
                ...
                if not isinstance(formats, tuple) or len(formats) != 2:
                    logger.error(
                        f"Plugin {plugin_class} supported_formats must return tuple of length 2"
                    )
                    return False
                ...
                if not isinstance(name, str) or not name:
                    logger.error(
                        f"Plugin {plugin_class} plugin_name must return non-empty string"
                    )
                    return False
        ...
        **options: Any,
        ...
        if plugin_name:
            ...
            if not matching_plugins:
                raise ValueError(
                    f"No plugin found for {from_format} -> {to_format} conversion"
                )
    ...
    def register_converter_plugin(
        self, name: str, plugin_class: type[ConverterPlugin]
    ) -> None:
