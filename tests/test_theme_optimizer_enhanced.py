...
class TestMaterialThemeOptimization:
    ...
    async def test_material_theme_optimization(
        ...
        results = await optimizer.optimize(
            sample_articles, sample_categories, temp_output_dir
        )
...
class TestCustomAssetsHandling:
    ...
    async def test_custom_css_handling(self, temp_output_dir: Path):
        ...
        optimizer = ThemeOptimizer(theme="material", custom_css=[str(custom_css_file)])
    ...
    async def test_custom_js_handling(self, temp_output_dir: Path):
        ...
        optimizer = ThemeOptimizer(theme="material", custom_js=[str(custom_js_file)])
    ...
    async def test_missing_custom_files_warning(self, temp_output_dir: Path):
        ...
        assert any(
            "Custom CSS file not found" in warning for warning in results["warnings"]
        )
        assert any(
            "Custom JS file not found" in warning for warning in results["warnings"]
        )
...
class TestIntegrationScenarios:
    ...
    async def test_directory_creation_edge_cases(self, temp_output_dir: Path):
        ...
        await optimizer.optimize([], [], temp_output_dir)
    ...
    def test_config_updates_merging(self):
        ...
        assert (
            len(config_updates["extra_javascript"]) >= 3
        )  # extra.js + mathjax + 2 custom
