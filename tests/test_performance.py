...
from d361.api import BulkOperationManager, TokenManager
from d361.archive import SqliteCache
...
class PerformanceBenchmark:
    ...
    def measure_time(self, func_name: str):
        ...
        Returns an async wrapper for coroutine functions and a sync wrapper for
        regular functions, so both can be measured and called naturally.
        """

        def _record(start_time, start_memory):
            end_time = time.perf_counter()
            end_memory = self.process.memory_info().rss / 1024 / 1024  # MB
            self.results.setdefault(func_name, []).append(
                {
            ...
            if asyncio.iscoroutinefunction(func):
                ...
                async def async_wrapper(*args, **kwargs):
                    ...
                    try:
                        ...
                        _record(start_time, start_memory)
...
def large_dataset():
    ...
    for i in range(1000):
        article = Article(
            ...
            content="Large content for performance testing. " * 100,  # ~3KB per article
...
class TestDatabasePerformance:
    ...
    @pytest.mark.asyncio
    async def test_sqlite_cache_insert_performance(
        self, test_database, performance_benchmark, large_dataset
    ):
        """Test SQLite cache bulk insert performance."""
        from d361.archive.cache import CacheConfig

        config = CacheConfig(db_path=test_database, max_size_mb=100, ttl_seconds=3600)
        ...
        async def bulk_insert():
            for i, article in enumerate(articles):
                await cache.set(
                    f"article:{article.id}", article.model_dump()
    ...
    async def test_full_text_search_performance(
        ...
        async with aiosqlite.connect(test_database) as db:
            ...
            # Bulk insert articles (archive_id is a required column)
            for article in articles[:100]:  # Test with 100 articles
                await db.execute(
                    """
                    INSERT INTO articles (id, archive_id, title, content, category_id, status, created_at, updated_at)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                """,
                    (
                        article.id,
            ...
            async def search_test():
                cursor = await db.execute(
                    """
                    SELECT rowid, title, snippet(articles_fts, -1, '<b>', '</b>', '...', 10)
    ...
    async def test_concurrent_database_access(
        self, test_database, performance_benchmark
    ):
        """Test concurrent database access performance."""
        from d361.archive.cache import CacheConfig

        config = CacheConfig(db_path=test_database, max_size_mb=50, ttl_seconds=300)
        cache = SqliteCache(config=config)
        await cache.start()
        ...
        async def concurrent_operations():
            ...
            for i in range(20):
                ...
                async def write_task(index):
                    for j in range(10):
                        await cache.set(
                            f"concurrent:{index}:{j}", {"value": f"data-{index}-{j}"}
                        )
            ...
            for i in range(30):
                ...
                async def read_task(index):
                    for j in range(5):
                        try:
                            await cache.get(f"concurrent:{index % 20}:{j}")
                        except:
...
class TestApiPerformance:
    ...
    async def test_bulk_operations_performance(
        self, performance_benchmark, large_dataset
    ):
        ...
        assert len(results.successful) >= 45  # Most operations should succeed
...
class TestMemoryPerformance:
        ...
        def create_models():
            ...
            for i in range(1000):
                article = Article(
                    ...
                    content="Large content for memory testing. "
                    * 200,  # ~6KB per article
    ...
    async def test_provider_memory_scaling(self, performance_benchmark):
        ...
        provider = MockProvider(
            num_articles=2000, num_categories=100, include_content=True
        )
...
class TestPerformanceOptimization:
    ...
    @pytest.mark.performance
    @pytest.mark.asyncio
    async def test_performance_optimizer_efficiency(
        self, performance_benchmark, test_data_dir
    ):
        """Test PerformanceOptimizer efficiency with real workloads."""
        from d361.utils.performance import CacheConfig, async_cache

        # In-memory cache (no directory) keeps the test fast and disk-free
        optimizer = PerformanceOptimizer(cache_config=CacheConfig())

        @async_cache(cache_name="perf_test", optimizer=optimizer)
        ...
        async def test_caching_efficiency():
            # First run - should cache results (all misses)
            first_results = await asyncio.gather(
                *(expensive_operation(f"data-{i}") for i in range(20))
            )

            # Second run - should use cached results (all hits)
        ...
        # Cached results must match the freshly computed ones
        assert len(first_results) == 20
        assert len(second_results) == 20
        assert first_results == second_results

        # Check optimizer performance metrics for cache hits on the second run
        metrics = optimizer.get_global_metrics()
        cache_stats = metrics["cache_perf_test"]
        assert cache_stats["hits"] > 0  # Should have cache hits

        await optimizer.clear_all_caches()
        ...
        async def simulate_load():
            ...
            for i in range(100):
                ...
                async def api_call(call_id):
                    ...
                    await mock_client.token_manager.record_success(
                        token, response_time=0.01
                    )
...
class TestMkDocsExportPerformance:
    ...
    @pytest.mark.performance
    @pytest.mark.asyncio
    async def test_mkdocs_export_large_project_timing(
        self, performance_benchmark, large_dataset, tmp_path
    ):
        ...
        async def export_large_project():
            # Mock the provider (export() consumes get_articles/get_categories) and
            # file I/O so the benchmark exercises processing without disk writes.
            with (
                patch.object(
                    exporter.provider,
                    "get_articles",
                    return_value=large_dataset["articles"],
                ),
                patch.object(
                    exporter.provider,
                    "get_categories",
                    return_value=large_dataset["categories"],
                ),
                patch.object(exporter.asset_manager, "process_assets", return_value=[]),
                patch.object(
                    exporter, "_validate_export", return_value={"valid": True}
                ),
        ...
        # Validate performance requirements (results is a list of measurements)
        measurement = performance_benchmark.results["mkdocs_export_1000_pages"][-1]
        export_time = measurement["execution_time"]
        memory_usage = measurement["memory_delta"]

        # Assert 5-minute (300 seconds) performance target
        assert export_time < 300, (
            f"Export took {export_time:.2f}s, exceeds 5-minute target"
        )

        # Additional performance validations
        assert memory_usage < 500, (
            f"Memory usage {memory_usage:.1f}MB exceeds reasonable limits"
        )
        ...
        print("   🎯 Performance Target: <300 seconds (5 minutes)")
        print(f"   ✅ Target Met: {export_time < 300}")
        print(f"   💾 Memory Usage: {memory_usage:.1f} MB")

    @pytest.mark.performance
    @pytest.mark.asyncio
    async def test_mkdocs_export_memory_efficiency(
        self, performance_benchmark, tmp_path
    ):
        ...
        # Run memory test
        await create_and_configure_exporter()
    ...
    async def test_mkdocs_export_scalability_benchmark(
        ...
        for size in test_sizes:
            ...
            async def export_dataset():
                # Exercise the real content-enhancement pipeline per article
                for article in dataset:
                    await exporter.content_enhancer.enhance_article(article)

            await export_dataset()

            measurement = performance_benchmark.results[f"mkdocs_export_{size}_pages"][
                -1
            ]
            scalability_results[size] = {
                "duration": measurement["execution_time"],
                "memory": measurement["memory_delta"],
            }
        ...
        for size, metrics in scalability_results.items():
            ...
            print(
                f"   📄 {size:4d} pages: {duration:6.2f}s ({per_page_time:4.1f}ms/page), {memory:5.1f}MB"
            )
        ...
        if len(scalability_results) >= 2:
            ...
            assert scalability_ratio < 1.5, (
                f"Poor scalability: {scalability_ratio:.2f}x slowdown per page"
            )
        ...
        if largest_size >= 1000:
            ...
            print(
                f"   🏆 1000+ Page Performance: {largest_duration:.2f}s (target: <300s)"
            )
            assert largest_duration < 300, (
                f"Large dataset export {largest_duration:.2f}s exceeds 5-minute target"
            )
...
class TestMkDocsTemplateSystem:
    ...
    async def test_mkdocs_base_template_rendering(self):
        ...
        minimal_context = {"site_name": "Test Site", "export_date": "2025-08-08"}

        config_yaml = await generator.generate_config_from_template(
            "mkdocs_base.yml.j2", minimal_context
        )
        ...
        print("✅ Base template minimal context: PASSED")
    ...
    async def test_mkdocs_base_template_full_context(self):
        ...
        full_context = {
            ...
            "navigation": ["- Home: index.md", "- Guide: guide.md"],
            "toc_depth": 4,
            ...
            "redirects": {"old-page.md": "new-page.md", "legacy/": "current/"},
            "social": [
        ...
        config_yaml = await generator.generate_config_from_template(
            "mkdocs_base.yml.j2", full_context
        )
        config = yaml.safe_load(config_yaml)
        ...
        assert any(
            "pymdownx.tasklist" in str(ext) for ext in config["markdown_extensions"]
        )
        assert any(
            "pymdownx.superfences" in str(ext) for ext in config["markdown_extensions"]
        )

        # Validate plugins with options
        plugins = config["plugins"]
        search_plugin = next(
            p for p in plugins if isinstance(p, dict) and "search" in p
        )
        ...
        print("✅ Base template full context: PASSED")
    ...
    async def test_material_theme_template_rendering(self):
        ...
        config_yaml = await generator.generate_config_from_template(
            "material_theme.yml.j2", material_context, validate_output=False
        )
        ...
        print("✅ Material theme template: PASSED")
    ...
    async def test_plugin_configs_template_rendering(self):
        ...
        plugin_context = {
            ...
            "minify_css": True,
            "enable_redirects": True,
            "redirect_maps": {"old/page.html": "new/page/", "legacy.md": "current.md"},
        }

        config_yaml = await generator.generate_config_from_template(
            "plugin_configs.yml.j2", plugin_context, validate_output=False
        )
        config = yaml.safe_load(config_yaml)
        ...
        plugin_names = [
            next(iter(p.keys())) if isinstance(p, dict) else p for p in plugins
        ]
        ...
        print("✅ Plugin configs template: PASSED")
    ...
    async def test_template_edge_cases_and_validation(self):
        ...
        try:
            ...
            print("✅ Minimal context handling: PASSED")
        ...
        try:
            config_yaml = await generator.generate_config_from_template(
                "mkdocs_base.yml.j2", none_context
            )
            config = yaml.safe_load(config_yaml)
            # Should handle None values gracefully
            assert (
                config["site_name"] == "None Values Test"
    ...
    async def test_template_filter_applications(self):
        ...
        config_yaml = await generator.generate_config_from_template(
            "mkdocs_base.yml.j2", filter_context
        )
        config = yaml.safe_load(config_yaml)
    ...
    async def test_template_conditional_rendering(self):
        ...
        config_yaml = await generator.generate_config_from_template(
            "mkdocs_base.yml.j2", disabled_context
        )
        config = yaml.safe_load(config_yaml)
        ...
        plugins = [
            next(iter(p.keys())) if isinstance(p, dict) else p
        ...
        print("✅ Conditional rendering (disabled): PASSED")
        ...
        config_yaml = await generator.generate_config_from_template(
            "mkdocs_base.yml.j2", enabled_context
        )
        config = yaml.safe_load(config_yaml)
        ...
        print("✅ Conditional rendering (enabled): PASSED")
