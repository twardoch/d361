...
class TestAppConfig:
    ...
    def test_app_config_production_validation(self):
        ...
        issues = config.validate_configuration()
        [issue for issue in issues if "debug" in issue.lower()]
    ...
    def test_app_config_environment_variables(self):
        ...
        with patch.dict(
            ...
            assert (
                config.app_name == "env-app" or config.app_name == "d361"
...
class TestEnvironmentLoader:
    ...
    def test_config_file_paths(self, test_data_dir, test_helpers):
        ...
        test_helpers.create_test_config_file(
            test_data_dir, "config.yaml", {"test": "value"}
        )
        test_helpers.create_test_config_file(
            test_data_dir, "config.production.yaml", {"prod": "value"}
        )
    ...
    def test_env_file_paths(self, test_data_dir, test_helpers):
        ...
        # Write .env.development directly (it's a file, not a dir)
        (test_data_dir / ".env.development").write_text("DEV=value\n")
    ...
    async def test_configuration_loading(self, test_data_dir, test_helpers):
        ...
        config_data = {"app_name": "test-app", "debug": True}
    ...
    def test_hot_reload_setup(self, test_data_dir):
        ...
        # Isolate this test's config dir. test_data_dir is session-scoped, so a
        # config.yaml written into it would leak into every other test's loader.
        hot_dir = test_data_dir / "hotreload"
        hot_dir.mkdir(exist_ok=True)
        # The watcher only monitors files that exist, so give it one to watch.
        (hot_dir / "config.yaml").write_text("app_name: test\n")

        loader = EnvironmentLoader(hot_dir)
        ...
        try:
            # A development config with a file present must start a watcher.
...
class TestSecretsProviders:
    ...
    async def test_local_file_secrets_provider_list(self, test_data_dir):
        ...
        # Dedicated dir: the session-scoped test_data_dir/secrets is shared with
        # test_local_file_secrets_provider, which would inflate the listed count.
        secrets_dir = test_data_dir / "secrets_list"
    ...
    async def test_secrets_provider_health_check(self, test_data_dir):
        ...
        # Local file provider
        file_provider = LocalFileSecretsProvider(
            {"secrets_dir": test_data_dir / "secrets"}
        )
...
class TestSecretsManager:
    ...
    async def test_secrets_manager_secret_operations(self, test_data_dir):
        ...
        metadata = await manager.set_secret(
            "test_api_token", "token_value_123", SecretType.API_TOKEN
        )
...
class TestConfigLoader:
    ...
    async def test_config_loader_initialization(self, test_data_dir):
        ...
        loader = ConfigLoader(
            base_config_dir=test_data_dir, enable_secrets=False, enable_hot_reload=False
        )
    ...
    async def test_config_loader_with_secrets(self, test_data_dir):
        ...
        loader = ConfigLoader(
            base_config_dir=test_data_dir, enable_secrets=True, enable_hot_reload=False
        )

        # Should initialize secrets manager
        await loader.load_configuration(Environment.TESTING)
...
class TestConfigValidator:
    ...
    def test_config_validator_resource_limits(self):
        ...
        [issue for issue in issues if "cache size" in issue.lower()]
    ...
    def test_config_validator_network_settings(self):
        ...
        [issue for issue in issues if "timeout" in issue.lower()]
...
class TestConfigurationEdgeCases:
    ...
    @pytest.mark.asyncio
    async def test_config_loading_with_corrupted_files(
        self, test_data_dir, test_helpers
    ):
        ...
        try:
            loader.load_configuration(Environment.TESTING)
    ...
    async def test_secrets_with_provider_failures(self):
        ...
        with patch(
            "d361.config.secrets_manager.create_secrets_provider",
...
def sample_config_data() -> dict[str, Any]:
    ...
    return {
        ...
        "environment": "testing",
        "debug": True,
        "api": {"timeout_seconds": 30, "max_retries": 3, "api_tokens": ["test-token"]},
        "cache": {"enabled": True, "max_memory_mb": 128},
...
def production_config_data() -> dict[str, Any]:
    ...
    return {
        ...
        "debug": False,
        "api": {"timeout_seconds": 60, "max_retries": 5, "api_tokens": ["prod-token"]},
        "monitoring": {
