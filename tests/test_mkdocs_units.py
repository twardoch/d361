...
class TestNavigationBuilderUnit:
    ...
    async def test_build_navigation_articles_only(self):
        ...
        assert (
            len(result["navigation"]) >= 0
        )  # Should handle articles without categories
...
```"""
        ...
        [line for line in lines if "linenums=" in line]
