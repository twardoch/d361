...
from d361.providers import ApiProvider, ArchiveProvider, HybridProvider, MockProvider
...
class TestMockProvider:
    ...
    def test_mock_provider_custom_parameters(self):
        ...
        provider = MockProvider(
            num_articles=20, num_categories=8, include_content=False
        )
    ...
    async def test_mock_provider_content_generation(self):
        ...
        article_without_content = await provider_without_content.get_article(
            "mock-article-1"
        )
    ...
    def test_mock_provider_consistency(self):
        ...
        MockProvider(num_articles=5, seed=12345)
        MockProvider(num_articles=5, seed=12345)
...
class TestArchiveProvider:
    ...
    async def test_archive_provider_list_articles(self, test_database):
        ...
        test_articles = [
            Article(
                id="archive-1", title="Archive Article 1", content="Archive content 1"
            ),
            Article(
                id="archive-2", title="Archive Article 2", content="Archive content 2"
            ),
        ]
...
class TestHybridProvider:
    ...
    async def test_hybrid_provider_caching_strategy(self):
        ...
        provider = HybridProvider(
            api_provider=mock_api, cache_provider=mock_cache, enable_caching=True
        )
    ...
    async def test_hybrid_provider_performance_metrics(self):
        ...
        provider = HybridProvider(api_provider=mock_api, collect_metrics=True)
