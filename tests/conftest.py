...
from d361.archive import ArchiveParser, SqliteCache
from d361.config import AppConfig, ConfigLoader, Environment, SecretsManager
...
from d361.utils import LoggingManager, PerformanceOptimizer, ServiceContainer
...
async def test_database(test_data_dir: Path) -> AsyncGenerator[Path, None]:
    ...
    # Unique per test: test_data_dir is session-scoped, so a constant filename
    # would share one DB file across every test and leak FTS/schema state.
    db_path = test_data_dir / f"test_d361_{os.urandom(8).hex()}.db"
...
async def sqlite_cache(
    test_database: Path, test_config: AppConfig
) -> AsyncGenerator[SqliteCache, None]:
    """Create SQLite cache for testing."""
    cache = SqliteCache(
        db_path=test_database, max_size_mb=TEST_CACHE_SIZE, ttl_seconds=300
    )
...
def mock_provider() -> MockProvider:
    ...
    provider = MockProvider(num_articles=5, num_categories=3, include_content=True)
    return provider
...
def performance_optimizer(test_data_dir: Path) -> PerformanceOptimizer:
    ...
    return PerformanceOptimizer(
        cache_dir=test_data_dir / "cache", max_cache_size_mb=TEST_CACHE_SIZE
    )
...
def mock_archive_file(test_data_dir: Path) -> Path:
    ...
    with zipfile.ZipFile(archive_path, "w") as zf:
        ...
        zf.writestr(
            "metadata.json", json.dumps({"version": "1.0", "created": "2025-01-01"})
        )
...
async def archive_parser(test_data_dir: Path) -> ArchiveParser:
    ...
    parser = ArchiveParser(
        cache_dir=test_data_dir / "archives", temp_dir=test_data_dir / "temp"
    )
...
class TestHelpers:
    ...
    def create_test_config_file(
        config_dir: Path, filename: str, config_data: dict[str, Any]
    ) -> Path:
