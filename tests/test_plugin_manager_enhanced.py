...
class TestPluginConfig:
    ...
    def test_plugin_config_to_dict_with_config(self):
        ...
        config = PluginConfig(
            name="minify", config={"minify_html": True, "minify_js": True}
        )

        result = config.to_dict()
        expected = {"minify": {"minify_html": True, "minify_js": True}}
...
class TestPluginSet:
    ...
    def test_plugin_set_creation(self):
        ...
        plugins = [PluginConfig(name="plugin1"), PluginConfig(name="plugin2")]
    ...
    def test_plugin_set_defaults(self):
        ...
        plugin_set = PluginSet(name="default_set", description="Default test set")
...
class TestOptimalPluginSelection:
    ...
    async def test_get_optimal_plugins_with_custom(self):
        ...
        plugins = await manager.get_optimal_plugins(custom_plugins=custom_plugins)
...
class TestPluginValidation:
    ...
    def test_validate_plugin_configuration_valid(self):
        ...
        plugins_config = ["search", {"autorefs": {}}, {"minify": {"minify_html": True}}]
    ...
    def test_validate_plugin_configuration_missing_search(self):
        ...
        plugins_config = [{"autorefs": {}}, {"minify": {"minify_html": True}}]
        ...
        assert any(
            "Search plugin missing" in warning for warning in results["warnings"]
        )
    ...
    def test_validate_plugin_configuration_suggestions(self):
        ...
        assert (
            "social" in suggestions_text
            or "minify" in suggestions_text
            or "offline" in suggestions_text
        )
...
class TestMarkdownExtensionManager:
    ...
    def test_get_optimal_extensions_minimal(self):
        ...
        extensions = manager.get_optimal_extensions(
            include_advanced=False, include_math=False, include_diagrams=False
        )
...
class TestIntegrationScenarios:
    ...
    async def test_complete_plugin_workflow(self):
        ...
        manager = PluginManager(
            theme="material", site_url="https://docs.example.com", enable_offline=True
        )

        # Get optimal plugins
        plugins = await manager.get_optimal_plugins(
            enable_seo=True, enable_performance=True, enable_advanced=True
        )
