...
async def test_extract_navigation_success(
    mock_config: Config, mock_page: MagicMock, sample_navigation_data: list[dict]
) -> None:
    ...
    with (
        patch("d361.offline.navigation.expand_navigation_tree") as mock_expand,
...
async def test_extract_navigation_no_tree_found(
    mock_config: Config, mock_page: MagicMock
) -> None:
...
def test_extract_tree_structure_simple() -> None:
    ...
    child1.query_selector.return_value.get_attribute.return_value = (
    ...
    child2.query_selector.return_value.get_attribute.return_value = (
